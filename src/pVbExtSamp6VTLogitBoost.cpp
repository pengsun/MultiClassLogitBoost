#include "pVbExtSamp6VTLogitBoost.hpp"
#include <functional>
#include <numeric>
...
// Implementation of pVbExtSamp6VTSplit
pVbExtSamp6VTSplit::pVbExtSamp6VTSplit()
{
  reset();
}

void pVbExtSamp6VTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pVbExtSamp6VTSolver
//const double pVbExtSamp6VTSolver::EPS = 0.01;
const double pVbExtSamp6VTSolver::MAXGAMMA = 5.0;
pVbExtSamp6VTSolver::pVbExtSamp6VTSolver( pVbExtSamp6VTData* _data, VecIdx* _ci)
{ 
  set_data(_data, _ci);
}

void pVbExtSamp6VTSolver::set_data( pVbExtSamp6VTData* _data, VecIdx* _ci)
{ 
  data_ = _data;
  ci_ = _ci;
...
void pVbExtSamp6VTSolver::update_internal( VecIdx& vidx )
{
...
void pVbExtSamp6VTSolver::update_internal_incre( int idx )
{
...
void pVbExtSamp6VTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pVbExtSamp6VTSolver::calc_gamma( double *gamma)
{
  int KK = mg_.size();
  for (int kk = 0; kk < KK; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
    //if (sh <= 0) cv::error("pVbExtSamp6VTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void pVbExtSamp6VTSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
// Implementation of pVbExtSamp6VTNode
pVbExtSamp6VTNode::pVbExtSamp6VTNode(int _K)
{
  id_ = 0;
...
pVbExtSamp6VTNode::pVbExtSamp6VTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pVbExtSamp6VTNode::calc_dir( float* _psample )
{
...
pVbExt5VT_best_split_finder::pVbExt5VT_best_split_finder(pVbExtSamp6VTTree *_tree, 
  pVbExtSamp6VTNode *_node, pVbExtSamp6VTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
// Implementation of pVbExtSamp6VTTree::Param
pVbExtSamp6VTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
...
// Implementation of pVbExtSamp6VTTree
void pVbExtSamp6VTTree::split( pVbExtSamp6VTData* _data )
{
  // clear
  clear();
  ...
  pVbExtSamp6VTNode* root = candidate_nodes_.top(); 
  ...
  {
    pVbExtSamp6VTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void pVbExtSamp6VTTree::fit( pVbExtSamp6VTData* _data )
{
  // fitting node data for each leaf
  std::list<pVbExtSamp6VTNode>::iterator it;
  for (it = nodes_.begin(); it != nodes_.end(); ++it) {
    pVbExtSamp6VTNode* nd = &(*it);
...
pVbExtSamp6VTNode* pVbExtSamp6VTTree::get_node( float* _sample)
{
  pVbExtSamp6VTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pVbExtSamp6VTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}

void pVbExtSamp6VTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pVbExtSamp6VTTree::predict( float* _sample, float* _score )
{
  ...
  pVbExtSamp6VTNode* nd = get_node(_sample);
  for (int k = 0; k < K_; ++k) {
...
void pVbExtSamp6VTTree::subsample(pVbExtSamp6VTData* _data)
{
  /// subsample samples
  // sample wise
...
void pVbExtSamp6VTTree::clear()
{
  nodes_.clear();
  candidate_nodes_.~priority_queue();
}

void pVbExtSamp6VTTree::creat_root_node( pVbExtSamp6VTData* _data )
{
  nodes_.push_back(pVbExtSamp6VTNode(0,K_));
  pVbExtSamp6VTNode* root = &(nodes_.back());
...
bool pVbExtSamp6VTTree::find_best_candidate_split( pVbExtSamp6VTNode* _node, pVbExtSamp6VTData* _data )
{
...
bool pVbExtSamp6VTTree::find_best_split_num_var( 
  pVbExtSamp6VTNode* _node, pVbExtSamp6VTData* _data, int _ivar, pVbExtSamp6VTSplit &cb_split)
{
  ...
  // initialize
  pVbExtSamp6VTSolver sol_left(_data, &(this->sub_ci_)), sol_right = _node->sol_this_;
...
void pVbExtSamp6VTTree::make_node_sorted_idx( pVbExtSamp6VTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pVbExtSamp6VTTree::set_best_split_num_var( 
  pVbExtSamp6VTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
  int best_i, double best_gain, double best_gain_left, double best_gain_right,
  pVbExtSamp6VTSplit &cb_split)
{
...
bool pVbExtSamp6VTTree::can_split_node( pVbExtSamp6VTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pVbExtSamp6VTTree::split_node( pVbExtSamp6VTNode* _node, pVbExtSamp6VTData* _data )
{
  // create left and right node
  pVbExtSamp6VTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  pVbExtSamp6VTNode tmp2(nodes_.size(), K_);
...
void pVbExtSamp6VTTree::calc_gain(pVbExtSamp6VTNode* _node, pVbExtSamp6VTData* _data)
{
  double gain;
...
void pVbExtSamp6VTTree::fit_node( pVbExtSamp6VTNode* _node, pVbExtSamp6VTData* _data )
{
  int nn = _node->sample_idx_.size();
...
// Implementation of pVbExtSamp6VTLogitBoost::Param
pVbExtSamp6VTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pVbExtSamp6VTLogitBoost
const double pVbExtSamp6VTLogitBoost::EPS_LOSS = 1e-14;
const double pVbExtSamp6VTLogitBoost::MAX_F = 100;
void pVbExtSamp6VTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pVbExtSamp6VTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pVbExtSamp6VTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pVbExtSamp6VTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pVbExtSamp6VTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pVbExtSamp6VTLogitBoost::get_class_count()
{
  return K_;
}

int pVbExtSamp6VTLogitBoost::get_num_iter()
{
  return NumIter_;
}

void pVbExtSamp6VTLogitBoost::get_nr( VecIdx& nr_wts, VecIdx& nr_wtc )
{
...
//double pVbExtSamp6VTLogitBoost::get_train_loss()
//{
//  if (NumIter_<1) return DBL_MAX;
//  return L_iter_.at<double>(NumIter_-1);
//}

void pVbExtSamp6VTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pVbExtSamp6VTLogitBoost::update_F(int t)
{
...
void pVbExtSamp6VTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
void pVbExtSamp6VTLogitBoost::update_gg()
{
  int N = F_.rows;
  double delta = 0;
...
//bool pVbExtSamp6VTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void pVbExtSamp6VTLogitBoost::calc_loss( MLData* _data )
{
...
void pVbExtSamp6VTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pVbExtSamp6VTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
