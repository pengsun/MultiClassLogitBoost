#include "pVbExtSamp5VTLogitBoost.hpp"
#include <functional>
#include <numeric>
...
pVbExtSamp5VTSplit::pVbExtSamp5VTSplit()
{
  reset();
}

void pVbExtSamp5VTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pVbExtSamp5VTSolver
//const double pVbExtSamp5VTSolver::EPS = 0.01;
const double pVbExtSamp5VTSolver::MAXGAMMA = 5.0;
pVbExtSamp5VTSolver::pVbExtSamp5VTSolver( pVbExtSamp5VTData* _data, VecIdx* _ci)
{ 
  set_data(_data, _ci);
}

void pVbExtSamp5VTSolver::set_data( pVbExtSamp5VTData* _data, VecIdx* _ci)
{ 
  data_ = _data;
  ci_ = _ci;
...
void pVbExtSamp5VTSolver::update_internal( VecIdx& vidx )
{
...
void pVbExtSamp5VTSolver::update_internal_incre( int idx )
{
...
void pVbExtSamp5VTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pVbExtSamp5VTSolver::calc_gamma( double *gamma)
{
  int KK = mg_.size();
  for (int kk = 0; kk < KK; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
    //if (sh <= 0) cv::error("pVbExtSamp5VTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void pVbExtSamp5VTSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
// Implementation of pVbExtSamp5VTNode
pVbExtSamp5VTNode::pVbExtSamp5VTNode(int _K)
{
  id_ = 0;
...
pVbExtSamp5VTNode::pVbExtSamp5VTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pVbExtSamp5VTNode::calc_dir( float* _psample )
{
...
pVbExt5VT_best_split_finder::pVbExt5VT_best_split_finder(pVbExtSamp5VTTree *_tree, 
  pVbExtSamp5VTNode *_node, pVbExtSamp5VTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
// Implementation of pVbExtSamp5VTTree::Param
pVbExtSamp5VTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
...
// Implementation of pVbExtSamp5VTTree
void pVbExtSamp5VTTree::split( pVbExtSamp5VTData* _data )
{
  // clear
  clear();
  ...
  pVbExtSamp5VTNode* root = candidate_nodes_.top(); 
  ...
  {
    pVbExtSamp5VTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void pVbExtSamp5VTTree::fit( pVbExtSamp5VTData* _data )
{
  // fitting node data for each leaf
  std::list<pVbExtSamp5VTNode>::iterator it;
  for (it = nodes_.begin(); it != nodes_.end(); ++it) {
    pVbExtSamp5VTNode* nd = &(*it);
...
pVbExtSamp5VTNode* pVbExtSamp5VTTree::get_node( float* _sample)
{
  pVbExtSamp5VTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pVbExtSamp5VTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}

void pVbExtSamp5VTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pVbExtSamp5VTTree::predict( float* _sample, float* _score )
{
  ...
  pVbExtSamp5VTNode* nd = get_node(_sample);
  for (int k = 0; k < K_; ++k) {
...
void pVbExtSamp5VTTree::subsample(pVbExtSamp5VTData* _data)
{
  /// subsample samples
  // sample wise
...
void pVbExtSamp5VTTree::clear()
{
  nodes_.clear();
  candidate_nodes_.~priority_queue();
}

void pVbExtSamp5VTTree::creat_root_node( pVbExtSamp5VTData* _data )
{
  nodes_.push_back(pVbExtSamp5VTNode(0,K_));
  pVbExtSamp5VTNode* root = &(nodes_.back());
...
bool pVbExtSamp5VTTree::find_best_candidate_split( pVbExtSamp5VTNode* _node, pVbExtSamp5VTData* _data )
{
...
bool pVbExtSamp5VTTree::find_best_split_num_var( 
  pVbExtSamp5VTNode* _node, pVbExtSamp5VTData* _data, int _ivar, pVbExtSamp5VTSplit &cb_split)
{
  ...
  // initialize
  pVbExtSamp5VTSolver sol_left(_data, &(this->sub_ci_)), sol_right = _node->sol_this_;
...
void pVbExtSamp5VTTree::make_node_sorted_idx( pVbExtSamp5VTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pVbExtSamp5VTTree::set_best_split_num_var( 
  pVbExtSamp5VTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
  int best_i, double best_gain, double best_gain_left, double best_gain_right,
  pVbExtSamp5VTSplit &cb_split)
{
...
bool pVbExtSamp5VTTree::can_split_node( pVbExtSamp5VTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pVbExtSamp5VTTree::split_node( pVbExtSamp5VTNode* _node, pVbExtSamp5VTData* _data )
{
  // create left and right node
  pVbExtSamp5VTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  pVbExtSamp5VTNode tmp2(nodes_.size(), K_);
...
void pVbExtSamp5VTTree::calc_gain(pVbExtSamp5VTNode* _node, pVbExtSamp5VTData* _data)
{
  double gain;
...
void pVbExtSamp5VTTree::fit_node( pVbExtSamp5VTNode* _node, pVbExtSamp5VTData* _data )
{
  int nn = _node->sample_idx_.size();
...
// Implementation of pVbExtSamp5VTLogitBoost::Param
pVbExtSamp5VTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pVbExtSamp5VTLogitBoost
const double pVbExtSamp5VTLogitBoost::EPS_LOSS = 1e-14;
const double pVbExtSamp5VTLogitBoost::MAX_F = 100;
void pVbExtSamp5VTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pVbExtSamp5VTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pVbExtSamp5VTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pVbExtSamp5VTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pVbExtSamp5VTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pVbExtSamp5VTLogitBoost::get_class_count()
{
  return K_;
}

int pVbExtSamp5VTLogitBoost::get_num_iter()
{
  return NumIter_;
}

void pVbExtSamp5VTLogitBoost::get_nr( VecIdx& nr_wts, VecIdx& nr_wtc )
{
...
//double pVbExtSamp5VTLogitBoost::get_train_loss()
//{
//  if (NumIter_<1) return DBL_MAX;
//  return L_iter_.at<double>(NumIter_-1);
//}

void pVbExtSamp5VTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pVbExtSamp5VTLogitBoost::update_F(int t)
{
...
void pVbExtSamp5VTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
void pVbExtSamp5VTLogitBoost::update_gg()
{
  int N = F_.rows;
  double delta = 0;
...
//bool pVbExtSamp5VTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void pVbExtSamp5VTLogitBoost::calc_loss( MLData* _data )
{
...
void pVbExtSamp5VTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pVbExtSamp5VTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
