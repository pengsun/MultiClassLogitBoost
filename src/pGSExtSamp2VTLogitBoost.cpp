#include "pGSExtSamp2VTLogitBoost.hpp"
#include <functional>
#include <numeric>
...
// Implementation of pGSExtSamp2VTSplit
pGSExtSamp2VTSplit::pGSExtSamp2VTSplit()
{
  reset();
}

void pGSExtSamp2VTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pGSExtSamp2VTSolver
//const double pGSExtSamp2VTSolver::EPS = 0.01;
const double pGSExtSamp2VTSolver::MAXGAMMA = 5.0;
pGSExtSamp2VTSolver::pGSExtSamp2VTSolver( pGSExtSamp2VTData* _data, VecIdx* _ci)
{ 
  set_data(_data, _ci);
}

void pGSExtSamp2VTSolver::set_data( pGSExtSamp2VTData* _data, VecIdx* _ci)
{ 
  data_ = _data;
  ci_ = _ci;
...
void pGSExtSamp2VTSolver::update_internal( VecIdx& vidx )
{
...
void pGSExtSamp2VTSolver::update_internal_incre( int idx )
{
...
void pGSExtSamp2VTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pGSExtSamp2VTSolver::calc_gamma( double *gamma)
{
  int KK = mg_.size();
  for (int kk = 0; kk < KK; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
    //if (sh <= 0) cv::error("pGSExtSamp2VTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void pGSExtSamp2VTSolver::calc_gain( double& gain )
{
  // Use just the gradient to calculate split gain
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
    double smg = mg_[k];
...
// Implementation of pGSExtSamp2VTNode
pGSExtSamp2VTNode::pGSExtSamp2VTNode(int _K)
{
  id_ = 0;
...
pGSExtSamp2VTNode::pGSExtSamp2VTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pGSExtSamp2VTNode::calc_dir( float* _psample )
{
...
// Implementation of pGSExt2VT_best_split_finder (helper class)
pGSExt2VT_best_split_finder::pGSExt2VT_best_split_finder(pGSExtSamp2VTTree *_tree, 
  pGSExtSamp2VTNode *_node, pGSExtSamp2VTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
pGSExt2VT_best_split_finder::pGSExt2VT_best_split_finder (const pGSExt2VT_best_split_finder &f, cv::Split)
{
  this->tree_ = f.tree_;
  this->node_ = f.node_;
  this->data_ = f.data_;
...
void pGSExt2VT_best_split_finder::operator() (const cv::BlockedRange &r)
{
...
void pGSExt2VT_best_split_finder::join (pGSExt2VT_best_split_finder &rhs)
{
...
// Implementation of pGSExtSamp2VTTree::Param
pGSExtSamp2VTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
...
// Implementation of pGSExtSamp2VTTree
void pGSExtSamp2VTTree::split( pGSExtSamp2VTData* _data )
{
  // clear
  clear();
  ...
  pGSExtSamp2VTNode* root = candidate_nodes_.top(); 
  ...
  {
    pGSExtSamp2VTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void pGSExtSamp2VTTree::fit( pGSExtSamp2VTData* _data )
{
  // fitting node data for each leaf
  std::list<pGSExtSamp2VTNode>::iterator it;
  for (it = nodes_.begin(); it != nodes_.end(); ++it) {
    pGSExtSamp2VTNode* nd = &(*it);
...
pGSExtSamp2VTNode* pGSExtSamp2VTTree::get_node( float* _sample)
{
  pGSExtSamp2VTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pGSExtSamp2VTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}

void pGSExtSamp2VTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pGSExtSamp2VTTree::predict( float* _sample, float* _score )
{
  ...
  pGSExtSamp2VTNode* nd = get_node(_sample);
  for (int k = 0; k < K_; ++k) {
...
void pGSExtSamp2VTTree::subsample(pGSExtSamp2VTData* _data)
{
  // subsample samples
  ...
  pGSExtSamp2VTSolver tmp_sol(_data, &all_ci);
...
void pGSExtSamp2VTTree::clear()
{
  nodes_.clear();
  candidate_nodes_.~priority_queue();
}

void pGSExtSamp2VTTree::creat_root_node( pGSExtSamp2VTData* _data )
{
  nodes_.push_back(pGSExtSamp2VTNode(0,K_));
  pGSExtSamp2VTNode* root = &(nodes_.back());
...
bool pGSExtSamp2VTTree::find_best_candidate_split( pGSExtSamp2VTNode* _node, pGSExtSamp2VTData* _data )
{
  ...
  pGSExt2VT_best_split_finder bsf(this,_node,_data);
...
bool pGSExtSamp2VTTree::find_best_split_num_var( 
  pGSExtSamp2VTNode* _node, pGSExtSamp2VTData* _data, int _ivar, pGSExtSamp2VTSplit &cb_split)
{
  ...
  // initialize
  pGSExtSamp2VTSolver sol_left(_data, &(this->sub_ci_)), sol_right = _node->sol_this_;
...
void pGSExtSamp2VTTree::make_node_sorted_idx( pGSExtSamp2VTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pGSExtSamp2VTTree::set_best_split_num_var( 
  pGSExtSamp2VTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
  int best_i, double best_gain, double best_gain_left, double best_gain_right,
  pGSExtSamp2VTSplit &cb_split)
{
...
bool pGSExtSamp2VTTree::can_split_node( pGSExtSamp2VTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pGSExtSamp2VTTree::split_node( pGSExtSamp2VTNode* _node, pGSExtSamp2VTData* _data )
{
  // create left and right node
  pGSExtSamp2VTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  pGSExtSamp2VTNode tmp2(nodes_.size(), K_);
...
void pGSExtSamp2VTTree::calc_gain(pGSExtSamp2VTNode* _node, pGSExtSamp2VTData* _data)
{
  double gain;
...
void pGSExtSamp2VTTree::fit_node( pGSExtSamp2VTNode* _node, pGSExtSamp2VTData* _data )
{
  int nn = _node->sample_idx_.size();
...
// Implementation of pGSExtSamp2VTLogitBoost::Param
pGSExtSamp2VTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pGSExtSamp2VTLogitBoost
const double pGSExtSamp2VTLogitBoost::EPS_LOSS = 1e-14;
const double pGSExtSamp2VTLogitBoost::MAX_F = 100;
void pGSExtSamp2VTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pGSExtSamp2VTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pGSExtSamp2VTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pGSExtSamp2VTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pGSExtSamp2VTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pGSExtSamp2VTLogitBoost::get_class_count()
{
  return K_;
}

int pGSExtSamp2VTLogitBoost::get_num_iter()
{
  return NumIter_;
}

//double pGSExtSamp2VTLogitBoost::get_train_loss()
//{
//  if (NumIter_<1) return DBL_MAX;
//  return L_iter_.at<double>(NumIter_-1);
//}

void pGSExtSamp2VTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pGSExtSamp2VTLogitBoost::update_F(int t)
{
...
void pGSExtSamp2VTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
//bool pGSExtSamp2VTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void pGSExtSamp2VTLogitBoost::calc_loss( MLData* _data )
{
...
void pGSExtSamp2VTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pGSExtSamp2VTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
