#include "pVbExtSamp13VTLogitBoost.hpp"
#include <functional>
#include <numeric>
...
// Implementation of pVbExtSamp13VTSplit
pVbExtSamp13VTSplit::pVbExtSamp13VTSplit()
{
  reset();
}

void pVbExtSamp13VTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pVbExtSamp13VTSolver
//const double pVbExtSamp13VTSolver::EPS = 0.01;
const double pVbExtSamp13VTSolver::MAXGAMMA = 5.0;
pVbExtSamp13VTSolver::pVbExtSamp13VTSolver( pVbExtSamp13VTData* _data, VecIdx* _ci)
{ 
  set_data(_data, _ci);
}

void pVbExtSamp13VTSolver::set_data( pVbExtSamp13VTData* _data, VecIdx* _ci)
{ 
  data_ = _data;
  ci_ = _ci;
...
void pVbExtSamp13VTSolver::update_internal( VecIdx& vidx )
{
...
void pVbExtSamp13VTSolver::update_internal_incre( int idx )
{
...
void pVbExtSamp13VTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pVbExtSamp13VTSolver::calc_gamma( double *gamma)
{
  int KK = mg_.size();
  for (int kk = 0; kk < KK; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
    //if (sh <= 0) cv::error("pVbExtSamp13VTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void pVbExtSamp13VTSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
pVbExtSamp13VTNode::pVbExtSamp13VTNode(int _K)
{
  id_ = 0;
...
pVbExtSamp13VTNode::pVbExtSamp13VTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pVbExtSamp13VTNode::calc_dir( float* _psample )
{
...
pVbExt13VT_best_split_finder::pVbExt13VT_best_split_finder(pVbExtSamp13VTTree *_tree, 
  pVbExtSamp13VTNode *_node, pVbExtSamp13VTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
// Implementation of pVbExtSamp13VTTree::Param
pVbExtSamp13VTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
...
// Implementation of pVbExtSamp13VTTree
void pVbExtSamp13VTTree::split( pVbExtSamp13VTData* _data )
{
  // clear
  clear();
  ...
  pVbExtSamp13VTNode* root = candidate_nodes_.top(); 
  ...
  {
    pVbExtSamp13VTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void pVbExtSamp13VTTree::fit( pVbExtSamp13VTData* _data )
{
  // fitting node data for each leaf
  std::list<pVbExtSamp13VTNode>::iterator it;
  for (it = nodes_.begin(); it != nodes_.end(); ++it) {
    pVbExtSamp13VTNode* nd = &(*it);
...
pVbExtSamp13VTNode* pVbExtSamp13VTTree::get_node( float* _sample)
{
  pVbExtSamp13VTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pVbExtSamp13VTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}
...
void pVbExtSamp13VTTree::get_is_leaf( VecInt& is_leaf )
{
  is_leaf.clear();
  is_leaf.resize(nodes_.size());

  std::list<pVbExtSamp13VTNode>::iterator it = nodes_.begin();
...
void pVbExtSamp13VTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pVbExtSamp13VTTree::predict( float* _sample, float* _score )
{
  ...
  pVbExtSamp13VTNode* nd = get_node(_sample);
  for (int k = 0; k < K_; ++k) {
...
void pVbExtSamp13VTTree::subsample_samples(pVbExtSamp13VTData* _data)
{
...
void pVbExtSamp13VTTree::subsample_classes_for_node( pVbExtSamp13VTNode* _node, pVbExtSamp13VTData* _data )
{
...
void pVbExtSamp13VTTree::creat_root_node( pVbExtSamp13VTData* _data )
{
  nodes_.push_back(pVbExtSamp13VTNode(0,K_));
  pVbExtSamp13VTNode* root = &(nodes_.back());
...
bool pVbExtSamp13VTTree::find_best_candidate_split( pVbExtSamp13VTNode* _node, pVbExtSamp13VTData* _data )
{
...
bool pVbExtSamp13VTTree::find_best_split_num_var( 
  pVbExtSamp13VTNode* _node, pVbExtSamp13VTData* _data, int _ivar, pVbExtSamp13VTSplit &cb_split)
{
  ...
  // initialize
  pVbExtSamp13VTSolver sol_left(_data, _node->sol_this_.ci_), 
...
void pVbExtSamp13VTTree::make_node_sorted_idx( pVbExtSamp13VTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pVbExtSamp13VTTree::set_best_split_num_var( 
  pVbExtSamp13VTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
...
bool pVbExtSamp13VTTree::can_split_node( pVbExtSamp13VTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pVbExtSamp13VTTree::split_node( pVbExtSamp13VTNode* _node, pVbExtSamp13VTData* _data )
{
  // create left and right node
  pVbExtSamp13VTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  pVbExtSamp13VTNode tmp2(nodes_.size(), K_);
...
void pVbExtSamp13VTTree::calc_gain(pVbExtSamp13VTNode* _node, pVbExtSamp13VTData* _data)
{
  double gain;
...
void pVbExtSamp13VTTree::fit_node( pVbExtSamp13VTNode* _node, pVbExtSamp13VTData* _data )
{
  int nn = _node->sample_idx_.size();
  if (nn<=0) return;
  ...
  for (int k = 0; k < this->K_; ++k)
    ci[k] = k;
  pVbExtSamp13VTSolver sol(_data, &ci);
...
// Implementation of pVbExtSamp13VTLogitBoost::Param
pVbExtSamp13VTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pVbExtSamp13VTLogitBoost
const double pVbExtSamp13VTLogitBoost::EPS_LOSS = 1e-6;
const double pVbExtSamp13VTLogitBoost::MAX_F = 100;
void pVbExtSamp13VTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pVbExtSamp13VTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pVbExtSamp13VTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pVbExtSamp13VTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pVbExtSamp13VTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pVbExtSamp13VTLogitBoost::get_class_count()
{
  return K_;
}

int pVbExtSamp13VTLogitBoost::get_num_iter()
{
  return NumIter_;
}

void pVbExtSamp13VTLogitBoost::get_nr( VecIdx& nr_wts, VecIdx& nr_wtc )
{
...
void pVbExtSamp13VTLogitBoost::get_cc( int itree, VecInt& node_cc )
{
...
void pVbExtSamp13VTLogitBoost::get_sc( int itree, VecInt& node_sc )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
//double pVbExtSamp13VTLogitBoost::get_train_loss()
//{
...
void pVbExtSamp13VTLogitBoost::get_is_leaf( int itree, VecInt& is_leaf )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
void pVbExtSamp13VTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pVbExtSamp13VTLogitBoost::update_F(int t)
{
...
void pVbExtSamp13VTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
void pVbExtSamp13VTLogitBoost::update_gg()
{
  int N = F_.rows;
  double delta = 0;
...
void pVbExtSamp13VTLogitBoost::calc_loss( MLData* _data )
{
...
void pVbExtSamp13VTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pVbExtSamp13VTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
