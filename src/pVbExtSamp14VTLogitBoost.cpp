...
// Implementation of pVbExtSamp14VTSplit
pVbExtSamp14VTSplit::pVbExtSamp14VTSplit()
{
  reset();
}

void pVbExtSamp14VTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pVbExtSamp14VTSolver
//const double pVbExtSamp14VTSolver::EPS = 0.01;
const double pVbExtSamp14VTSolver::MAXGAMMA = 5.0;
pVbExtSamp14VTSolver::pVbExtSamp14VTSolver( pVbExtSamp14VTData* _data, VecIdx* _ci)
{ 
  set_data(_data, _ci);
}

void pVbExtSamp14VTSolver::set_data( pVbExtSamp14VTData* _data, VecIdx* _ci)
{ 
  data_ = _data;
  ci_ = _ci;
...
void pVbExtSamp14VTSolver::update_internal( VecIdx& vidx )
{
...
void pVbExtSamp14VTSolver::update_internal_incre( int idx )
{
...
void pVbExtSamp14VTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pVbExtSamp14VTSolver::calc_gamma( double *gamma)
{
  int KK = mg_.size();
  for (int kk = 0; kk < KK; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
    //if (sh <= 0) cv::error("pVbExtSamp14VTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void pVbExtSamp14VTSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
pVbExtSamp14VTNode::pVbExtSamp14VTNode(int _K)
{
  id_ = 0;
...
pVbExtSamp14VTNode::pVbExtSamp14VTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pVbExtSamp14VTNode::calc_dir( float* _psample )
{
...
// Implementation of pVbExt14VT_best_split_finder (helper class)
pVbExt14VT_best_split_finder::pVbExt14VT_best_split_finder(pVbExtSamp14VTTree *_tree, 
  pVbExtSamp14VTNode *_node, pVbExtSamp14VTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
pVbExt14VT_best_split_finder::pVbExt14VT_best_split_finder (const pVbExt14VT_best_split_finder &f, cv::Split)
{
  this->tree_ = f.tree_;
  this->node_ = f.node_;
  this->data_ = f.data_;
...
void pVbExt14VT_best_split_finder::operator() (const cv::BlockedRange &r)
{
...
void pVbExt14VT_best_split_finder::join (pVbExt14VT_best_split_finder &rhs)
{
...
// Implementation of pVbExtSamp14VTTree::Param
pVbExtSamp14VTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
...
// Implementation of pVbExtSamp14VTTree
void pVbExtSamp14VTTree::split( pVbExtSamp14VTData* _data )
{
  // clear
  clear();
  ...
  pVbExtSamp14VTNode* root = candidate_nodes_.top(); 
  ...
  {
    pVbExtSamp14VTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void pVbExtSamp14VTTree::fit( pVbExtSamp14VTData* _data )
{
  // fitting node data for each leaf
  std::list<pVbExtSamp14VTNode>::iterator it;
  for (it = nodes_.begin(); it != nodes_.end(); ++it) {
    pVbExtSamp14VTNode* nd = &(*it);
...
pVbExtSamp14VTNode* pVbExtSamp14VTTree::get_node( float* _sample)
{
  pVbExtSamp14VTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pVbExtSamp14VTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}
...
void pVbExtSamp14VTTree::get_is_leaf( VecInt& is_leaf )
{
  is_leaf.clear();
  is_leaf.resize(nodes_.size());

  std::list<pVbExtSamp14VTNode>::iterator it = nodes_.begin();
...
void pVbExtSamp14VTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pVbExtSamp14VTTree::predict( float* _sample, float* _score )
{
  ...
  pVbExtSamp14VTNode* nd = get_node(_sample);
  for (int k = 0; k < K_; ++k) {
...
void pVbExtSamp14VTTree::subsample_samples(pVbExtSamp14VTData* _data)
{
...
void pVbExtSamp14VTTree::subsample_classes_for_node( pVbExtSamp14VTNode* _node, pVbExtSamp14VTData* _data )
{
...
void pVbExtSamp14VTTree::creat_root_node( pVbExtSamp14VTData* _data )
{
  nodes_.push_back(pVbExtSamp14VTNode(0,K_));
  pVbExtSamp14VTNode* root = &(nodes_.back());
...
bool pVbExtSamp14VTTree::find_best_candidate_split( pVbExtSamp14VTNode* _node, pVbExtSamp14VTData* _data )
{
  ...
  pVbExt14VT_best_split_finder bsf(this,_node,_data);
...
bool pVbExtSamp14VTTree::find_best_split_num_var( 
  pVbExtSamp14VTNode* _node, pVbExtSamp14VTData* _data, int _ivar, pVbExtSamp14VTSplit &cb_split)
{
  ...
  // initialize
  pVbExtSamp14VTSolver sol_left(_data, _node->sol_this_.ci_), 
...
void pVbExtSamp14VTTree::make_node_sorted_idx( pVbExtSamp14VTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pVbExtSamp14VTTree::set_best_split_num_var( 
  pVbExtSamp14VTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
...
bool pVbExtSamp14VTTree::can_split_node( pVbExtSamp14VTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pVbExtSamp14VTTree::split_node( pVbExtSamp14VTNode* _node, pVbExtSamp14VTData* _data )
{
  // create left and right node
  pVbExtSamp14VTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  pVbExtSamp14VTNode tmp2(nodes_.size(), K_);
...
void pVbExtSamp14VTTree::fit_node( pVbExtSamp14VTNode* _node, pVbExtSamp14VTData* _data )
{
  int nn = _node->sample_idx_.size();
  if (nn<=0) return;
...
void pVbExtSamp14VTTree::calc_node_gain(pVbExtSamp14VTNode* _node, pVbExtSamp14VTData* _data)
{
  double gain;
...
void pVbExtSamp14VTTree::calc_allsample_node_gain( pVbExtSamp14VTNode* _node, pVbExtSamp14VTData* _data )
{
...
// Implementation of pVbExtSamp14VTLogitBoost::Param
pVbExtSamp14VTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pVbExtSamp14VTLogitBoost
const double pVbExtSamp14VTLogitBoost::EPS_LOSS = 1e-6;
const double pVbExtSamp14VTLogitBoost::MAX_F = 100;
void pVbExtSamp14VTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pVbExtSamp14VTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pVbExtSamp14VTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pVbExtSamp14VTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pVbExtSamp14VTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pVbExtSamp14VTLogitBoost::get_class_count()
{
  return K_;
}

int pVbExtSamp14VTLogitBoost::get_num_iter()
{
  return NumIter_;
}

void pVbExtSamp14VTLogitBoost::get_nr( VecIdx& nr_wts, VecIdx& nr_wtc )
{
...
void pVbExtSamp14VTLogitBoost::get_cc( int itree, VecInt& node_cc )
{
...
void pVbExtSamp14VTLogitBoost::get_sc( int itree, VecInt& node_sc )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
//double pVbExtSamp14VTLogitBoost::get_train_loss()
//{
...
void pVbExtSamp14VTLogitBoost::get_is_leaf( int itree, VecInt& is_leaf )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
void pVbExtSamp14VTLogitBoost::get_all_sc( int itree, VecInt& node_all_sc )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
void pVbExtSamp14VTLogitBoost::get_gain( int itree, VecDbl& leaf_gain )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
void pVbExtSamp14VTLogitBoost::get_allsample_gain( int itree, VecDbl& leaf_allsample_gain )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
void pVbExtSamp14VTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pVbExtSamp14VTLogitBoost::update_F(int t)
{
...
void pVbExtSamp14VTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
void pVbExtSamp14VTLogitBoost::update_gg()
{
  int N = F_.rows;
  double delta = 0;
...
void pVbExtSamp14VTLogitBoost::update_abs_grad_class(int t)
{
  int N = F_.rows;
...
//bool pVbExtSamp14VTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void pVbExtSamp14VTLogitBoost::calc_loss( MLData* _data )
{
...
void pVbExtSamp14VTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pVbExtSamp14VTLogitBoost::calc_loss_class(MLData* _data,  int t )
{
  const double PMIN = 0.0001;
  int N = _data->X.rows;
  for (int i = 0; i < N; ++i) {
...
void pVbExtSamp14VTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
