#include "pVbExtSamp10VTLogitBoost.hpp"
#include <functional>
#include <numeric>
...
// Implementation of pVbExtSamp10VTSplit
pVbExtSamp10VTSplit::pVbExtSamp10VTSplit()
{
  reset();
}

void pVbExtSamp10VTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pVbExtSamp10VTSolver
//const double pVbExtSamp10VTSolver::EPS = 0.01;
const double pVbExtSamp10VTSolver::MAXGAMMA = 5.0;
pVbExtSamp10VTSolver::pVbExtSamp10VTSolver( pVbExtSamp10VTData* _data, VecIdx* _ci)
{ 
  set_data(_data, _ci);
}

void pVbExtSamp10VTSolver::set_data( pVbExtSamp10VTData* _data, VecIdx* _ci)
{ 
  data_ = _data;
  ci_ = _ci;
...
void pVbExtSamp10VTSolver::update_internal( VecIdx& vidx )
{
...
void pVbExtSamp10VTSolver::update_internal_incre( int idx )
{
...
void pVbExtSamp10VTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pVbExtSamp10VTSolver::calc_gamma( double *gamma)
{
  int KK = mg_.size();
  for (int kk = 0; kk < KK; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
    //if (sh <= 0) cv::error("pVbExtSamp10VTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void pVbExtSamp10VTSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
// Implementation of pVbExtSamp10VTNode
pVbExtSamp10VTNode::pVbExtSamp10VTNode(int _K)
{
  id_ = 0;
...
pVbExtSamp10VTNode::pVbExtSamp10VTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pVbExtSamp10VTNode::calc_dir( float* _psample )
{
...
// Implementation of pVbExt10VT_best_split_finder (helper class)
pVbExt10VT_best_split_finder::pVbExt10VT_best_split_finder(pVbExtSamp10VTTree *_tree, 
  pVbExtSamp10VTNode *_node, pVbExtSamp10VTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
pVbExt10VT_best_split_finder::pVbExt10VT_best_split_finder (const pVbExt10VT_best_split_finder &f, cv::Split)
{
  this->tree_ = f.tree_;
  this->node_ = f.node_;
  this->data_ = f.data_;
...
void pVbExt10VT_best_split_finder::operator() (const cv::BlockedRange &r)
{
...
void pVbExt10VT_best_split_finder::join (pVbExt10VT_best_split_finder &rhs)
{
...
// Implementation of pVbExtSamp10VTTree::Param
pVbExtSamp10VTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
...
// Implementation of pVbExtSamp10VTTree
void pVbExtSamp10VTTree::split( pVbExtSamp10VTData* _data )
{
  // clear
  clear();
  ...
  pVbExtSamp10VTNode* root = candidate_nodes_.top(); 
  ...
  {
    pVbExtSamp10VTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void pVbExtSamp10VTTree::fit( pVbExtSamp10VTData* _data )
{
  // fitting node data for each leaf
  std::list<pVbExtSamp10VTNode>::iterator it;
  for (it = nodes_.begin(); it != nodes_.end(); ++it) {
    pVbExtSamp10VTNode* nd = &(*it);
...
pVbExtSamp10VTNode* pVbExtSamp10VTTree::get_node( float* _sample)
{
  pVbExtSamp10VTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pVbExtSamp10VTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}

void pVbExtSamp10VTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pVbExtSamp10VTTree::predict( float* _sample, float* _score )
{
  ...
  pVbExtSamp10VTNode* nd = get_node(_sample);
  for (int k = 0; k < K_; ++k) {
...
void pVbExtSamp10VTTree::subsample(pVbExtSamp10VTData* _data)
{
  /// subsample samples
  // sample wise
...
void pVbExtSamp10VTTree::subsample_classes_for_node( pVbExtSamp10VTNode* _node, pVbExtSamp10VTData* _data )
{
  /// subsample classes
...
void pVbExtSamp10VTTree::clear()
{
  nodes_.clear();
  candidate_nodes_.~priority_queue();
}

void pVbExtSamp10VTTree::creat_root_node( pVbExtSamp10VTData* _data )
{
  nodes_.push_back(pVbExtSamp10VTNode(0,K_));
  pVbExtSamp10VTNode* root = &(nodes_.back());
...
bool pVbExtSamp10VTTree::find_best_candidate_split( pVbExtSamp10VTNode* _node, pVbExtSamp10VTData* _data )
{
  ...
  pVbExt10VT_best_split_finder bsf(this,_node,_data);
...
bool pVbExtSamp10VTTree::find_best_split_num_var( 
  pVbExtSamp10VTNode* _node, pVbExtSamp10VTData* _data, int _ivar, pVbExtSamp10VTSplit &cb_split)
{
  ...
  // initialize
  pVbExtSamp10VTSolver sol_left(_data, _node->sol_this_.ci_), 
...
void pVbExtSamp10VTTree::make_node_sorted_idx( pVbExtSamp10VTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pVbExtSamp10VTTree::set_best_split_num_var( 
  pVbExtSamp10VTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
...
bool pVbExtSamp10VTTree::can_split_node( pVbExtSamp10VTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pVbExtSamp10VTTree::split_node( pVbExtSamp10VTNode* _node, pVbExtSamp10VTData* _data )
{
  // create left and right node
  pVbExtSamp10VTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  pVbExtSamp10VTNode tmp2(nodes_.size(), K_);
...
void pVbExtSamp10VTTree::calc_gain(pVbExtSamp10VTNode* _node, pVbExtSamp10VTData* _data)
{
  double gain;
...
void pVbExtSamp10VTTree::fit_node( pVbExtSamp10VTNode* _node, pVbExtSamp10VTData* _data )
{
  int nn = _node->sample_idx_.size();
  if (nn<=0) return;
  ...
  for (int k = 0; k < this->K_; ++k)
    ci[k] = k;
  pVbExtSamp10VTSolver sol(_data, &ci);
...
// Implementation of pVbExtSamp10VTLogitBoost::Param
pVbExtSamp10VTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pVbExtSamp10VTLogitBoost
const double pVbExtSamp10VTLogitBoost::EPS_LOSS = 1e-14;
const double pVbExtSamp10VTLogitBoost::MAX_F = 100;
void pVbExtSamp10VTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pVbExtSamp10VTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pVbExtSamp10VTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pVbExtSamp10VTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pVbExtSamp10VTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pVbExtSamp10VTLogitBoost::get_class_count()
{
  return K_;
}

int pVbExtSamp10VTLogitBoost::get_num_iter()
{
  return NumIter_;
}

void pVbExtSamp10VTLogitBoost::get_nr( VecIdx& nr_wts, VecIdx& nr_wtc )
{
...
void pVbExtSamp10VTLogitBoost::get_cc( int itree, VecInt& node_cc )
{
...
//double pVbExtSamp10VTLogitBoost::get_train_loss()
//{
//  if (NumIter_<1) return DBL_MAX;
//  return L_iter_.at<double>(NumIter_-1);
//}

void pVbExtSamp10VTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pVbExtSamp10VTLogitBoost::update_F(int t)
{
...
void pVbExtSamp10VTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
void pVbExtSamp10VTLogitBoost::update_gg()
{
  int N = F_.rows;
  double delta = 0;
...
//bool pVbExtSamp10VTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void pVbExtSamp10VTLogitBoost::calc_loss( MLData* _data )
{
...
void pVbExtSamp10VTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pVbExtSamp10VTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
