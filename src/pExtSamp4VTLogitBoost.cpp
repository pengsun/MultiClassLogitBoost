#include "pExtSamp4VTLogitBoost.hpp"
#include <functional>
#include <numeric>
...
// Implementation of pExtSamp4VTSplit
pExtSamp4VTSplit::pExtSamp4VTSplit()
{
  reset();
}

void pExtSamp4VTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pExtSamp4VTSolver
//const double pExtSamp4VTSolver::EPS = 0.01;
const double pExtSamp4VTSolver::MAXGAMMA = 5.0;
pExtSamp4VTSolver::pExtSamp4VTSolver( pExtSamp4VTData* _data, VecIdx* _ci)
{ 
  set_data(_data, _ci);
}

void pExtSamp4VTSolver::set_data( pExtSamp4VTData* _data, VecIdx* _ci)
{ 
  data_ = _data;
  ci_ = _ci;
...
void pExtSamp4VTSolver::update_internal( VecIdx& vidx )
{
...
void pExtSamp4VTSolver::update_internal_incre( int idx )
{
...
void pExtSamp4VTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pExtSamp4VTSolver::calc_gamma( double *gamma)
{
  int KK = mg_.size();
  for (int kk = 0; kk < KK; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
    //if (sh <= 0) cv::error("pExtSamp4VTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void pExtSamp4VTSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
// Implementation of pExtSamp4VTNode
pExtSamp4VTNode::pExtSamp4VTNode(int _K)
{
  id_ = 0;
...
pExtSamp4VTNode::pExtSamp4VTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pExtSamp4VTNode::calc_dir( float* _psample )
{
...
// Implementation of pExt4VT_best_split_finder (helper class)
pExt4VT_best_split_finder::pExt4VT_best_split_finder(pExtSamp4VTTree *_tree, 
  pExtSamp4VTNode *_node, pExtSamp4VTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
pExt4VT_best_split_finder::pExt4VT_best_split_finder (const pExt4VT_best_split_finder &f, cv::Split)
{
  this->tree_ = f.tree_;
  this->node_ = f.node_;
  this->data_ = f.data_;
...
void pExt4VT_best_split_finder::operator() (const cv::BlockedRange &r)
{
...
void pExt4VT_best_split_finder::join (pExt4VT_best_split_finder &rhs)
{
...
// Implementation of pExtSamp4VTTree::Param
pExtSamp4VTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
...
// Implementation of pExtSamp4VTTree
void pExtSamp4VTTree::split( pExtSamp4VTData* _data )
{
  // clear
  clear();
  ...
  pExtSamp4VTNode* root = candidate_nodes_.top(); 
  ...
  {
    pExtSamp4VTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void pExtSamp4VTTree::fit( pExtSamp4VTData* _data )
{
  // fitting node data for each leaf
  std::list<pExtSamp4VTNode>::iterator it;
  for (it = nodes_.begin(); it != nodes_.end(); ++it) {
    pExtSamp4VTNode* nd = &(*it);
...
pExtSamp4VTNode* pExtSamp4VTTree::get_node( float* _sample)
{
  pExtSamp4VTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pExtSamp4VTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}

void pExtSamp4VTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pExtSamp4VTTree::predict( float* _sample, float* _score )
{
  ...
  pExtSamp4VTNode* nd = get_node(_sample);
  for (int k = 0; k < K_; ++k) {
...
void pExtSamp4VTTree::subsample(pExtSamp4VTData* _data)
{
  // subsample samples
...
#endif
  weight_trim_ratio ( *(_data->nd_), this->param_.ratio_si_, this->sub_si_);
  ...
  pExtSamp4VTSolver tmp_sol(_data, &all_ci);
...
void pExtSamp4VTTree::clear()
{
  nodes_.clear();
  candidate_nodes_.~priority_queue();
}

void pExtSamp4VTTree::creat_root_node( pExtSamp4VTData* _data )
{
  nodes_.push_back(pExtSamp4VTNode(0,K_));
  pExtSamp4VTNode* root = &(nodes_.back());
...
bool pExtSamp4VTTree::find_best_candidate_split( pExtSamp4VTNode* _node, pExtSamp4VTData* _data )
{
  ...
  pExt4VT_best_split_finder bsf(this,_node,_data);
...
bool pExtSamp4VTTree::find_best_split_num_var( 
  pExtSamp4VTNode* _node, pExtSamp4VTData* _data, int _ivar, pExtSamp4VTSplit &cb_split)
{
  ...
  // initialize
  pExtSamp4VTSolver sol_left(_data, &(this->sub_ci_)), sol_right = _node->sol_this_;
...
void pExtSamp4VTTree::make_node_sorted_idx( pExtSamp4VTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pExtSamp4VTTree::set_best_split_num_var( 
  pExtSamp4VTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
...
bool pExtSamp4VTTree::can_split_node( pExtSamp4VTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pExtSamp4VTTree::split_node( pExtSamp4VTNode* _node, pExtSamp4VTData* _data )
{
  // create left and right node
  pExtSamp4VTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  pExtSamp4VTNode tmp2(nodes_.size(), K_);
...
void pExtSamp4VTTree::calc_gain(pExtSamp4VTNode* _node, pExtSamp4VTData* _data)
{
  double gain;
...
void pExtSamp4VTTree::fit_node( pExtSamp4VTNode* _node, pExtSamp4VTData* _data )
{
  int nn = _node->sample_idx_.size();
...
// Implementation of pExtSamp4VTLogitBoost::Param
pExtSamp4VTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pExtSamp4VTLogitBoost
const double pExtSamp4VTLogitBoost::EPS_LOSS = 1e-14;
const double pExtSamp4VTLogitBoost::MAX_F = 100;
void pExtSamp4VTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pExtSamp4VTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pExtSamp4VTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pExtSamp4VTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pExtSamp4VTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pExtSamp4VTLogitBoost::get_class_count()
{
  return K_;
}

int pExtSamp4VTLogitBoost::get_num_iter()
{
  return NumIter_;
}

//double pExtSamp4VTLogitBoost::get_train_loss()
//{
//  if (NumIter_<1) return DBL_MAX;
//  return L_iter_.at<double>(NumIter_-1);
//}

void pExtSamp4VTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pExtSamp4VTLogitBoost::update_F(int t)
{
...
void pExtSamp4VTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
void pExtSamp4VTLogitBoost::update_nd()
{
  int N = p_.rows;
  const double MAX_ND = 100.0;
  ...
  for (int i = 0; i < N; ++i) {
    ...
    // update nd_
    if ( ggg > (MAX_ND*K_)) ggg = MAX_ND*K_;
    nd_.at<double>(i) = ggg;
  }

}

//bool pExtSamp4VTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void pExtSamp4VTLogitBoost::calc_loss( MLData* _data )
{
...
void pExtSamp4VTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pExtSamp4VTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
