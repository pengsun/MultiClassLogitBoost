#include "pAOSOLogitBoostV2Vb.hpp"
#include <functional>
#include <numeric>
...
// Implementation of pAOSOLogitBoostV2Vb::Param
pAOSOLogitBoostV2Vb::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pAOSOLogitBoostV2Vb
const double pAOSOLogitBoostV2Vb::EPS_LOSS = 1e-6;
const double pAOSOLogitBoostV2Vb::MAX_F = 100;
void pAOSOLogitBoostV2Vb::train( MLData* _data )
{
  train_init(_data);
...
void pAOSOLogitBoostV2Vb::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pAOSOLogitBoostV2Vb::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pAOSOLogitBoostV2Vb::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pAOSOLogitBoostV2Vb::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pAOSOLogitBoostV2Vb::get_class_count()
{
  return K_;
}

int pAOSOLogitBoostV2Vb::get_num_iter()
{
  return NumIter_;
}

void pAOSOLogitBoostV2Vb::get_nr( VecIdx& nr_wts, VecIdx& nr_wtc )
{
...
void pAOSOLogitBoostV2Vb::get_cc( int itree, VecInt& node_cc )
{
...
void pAOSOLogitBoostV2Vb::get_sc( int itree, VecInt& node_sc )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
void pAOSOLogitBoostV2Vb::get_is_leaf( int itree, VecInt& is_leaf )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
void pAOSOLogitBoostV2Vb::update_F(int t)
{
...
void pAOSOLogitBoostV2Vb::update_p()
{
  int N = F_.rows;
  int K = K_;
...
void pAOSOLogitBoostV2Vb::update_gg()
{
  int N = F_.rows;
  double delta = 0;
...
void pAOSOLogitBoostV2Vb::update_abs_grad_class(int t)
{
  int N = F_.rows;
...
void pAOSOLogitBoostV2Vb::calc_loss( MLData* _data )
{
...
void pAOSOLogitBoostV2Vb::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pAOSOLogitBoostV2Vb::calc_loss_class(MLData* _data,  int t )
{
  const double PMIN = 0.0001;
  int N = _data->X.rows;
  for (int i = 0; i < N; ++i) {
...
bool pAOSOLogitBoostV2Vb::should_stop( int t )
{
...
void pAOSOLogitBoostV2Vb::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
