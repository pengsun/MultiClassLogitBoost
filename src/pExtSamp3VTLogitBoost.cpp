#include "pExtSamp3VTLogitBoost.hpp"
#include <functional>
#include <numeric>
...
// Implementation of pExtSamp3VTSplit
pExtSamp3VTSplit::pExtSamp3VTSplit()
{
  reset();
}

void pExtSamp3VTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pExtSamp3VTSolver
//const double pExtSamp3VTSolver::EPS = 0.01;
const double pExtSamp3VTSolver::MAXGAMMA = 5.0;
pExtSamp3VTSolver::pExtSamp3VTSolver( pExtSamp3VTData* _data, VecIdx* _ci)
{ 
  set_data(_data, _ci);
}

void pExtSamp3VTSolver::set_data( pExtSamp3VTData* _data, VecIdx* _ci)
{ 
  data_ = _data;
  ci_ = _ci;
...
void pExtSamp3VTSolver::update_internal( VecIdx& vidx )
{
...
void pExtSamp3VTSolver::update_internal_incre( int idx )
{
...
void pExtSamp3VTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pExtSamp3VTSolver::calc_gamma( double *gamma)
{
  int KK = mg_.size();
  for (int kk = 0; kk < KK; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
    //if (sh <= 0) cv::error("pExtSamp3VTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void pExtSamp3VTSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
// Implementation of pExtSamp3VTNode
pExtSamp3VTNode::pExtSamp3VTNode(int _K)
{
  id_ = 0;
...
pExtSamp3VTNode::pExtSamp3VTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pExtSamp3VTNode::calc_dir( float* _psample )
{
...
// Implementation of pExt3VT_best_split_finder (helper class)
pExt3VT_best_split_finder::pExt3VT_best_split_finder(pExtSamp3VTTree *_tree, 
  pExtSamp3VTNode *_node, pExtSamp3VTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
pExt3VT_best_split_finder::pExt3VT_best_split_finder (const pExt3VT_best_split_finder &f, cv::Split)
{
  this->tree_ = f.tree_;
  this->node_ = f.node_;
  this->data_ = f.data_;
...
void pExt3VT_best_split_finder::operator() (const cv::BlockedRange &r)
{
...
void pExt3VT_best_split_finder::join (pExt3VT_best_split_finder &rhs)
{
...
// Implementation of pExtSamp3VTTree::Param
pExtSamp3VTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
...
// Implementation of pExtSamp3VTTree
void pExtSamp3VTTree::split( pExtSamp3VTData* _data )
{
  // clear
  clear();
  ...
  pExtSamp3VTNode* root = candidate_nodes_.top(); 
  ...
  {
    pExtSamp3VTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void pExtSamp3VTTree::fit( pExtSamp3VTData* _data )
{
  // fitting node data for each leaf
  std::list<pExtSamp3VTNode>::iterator it;
  for (it = nodes_.begin(); it != nodes_.end(); ++it) {
    pExtSamp3VTNode* nd = &(*it);
...
pExtSamp3VTNode* pExtSamp3VTTree::get_node( float* _sample)
{
  pExtSamp3VTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pExtSamp3VTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}

void pExtSamp3VTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pExtSamp3VTTree::predict( float* _sample, float* _score )
{
  ...
  pExtSamp3VTNode* nd = get_node(_sample);
  for (int k = 0; k < K_; ++k) {
...
void pExtSamp3VTTree::subsample(pExtSamp3VTData* _data)
{
  // subsample samples
...
#endif
  weight_trim_ratio ( *(_data->w_), this->param_.ratio_si_, this->sub_si_);
  ...
  pExtSamp3VTSolver tmp_sol(_data, &all_ci);
...
void pExtSamp3VTTree::clear()
{
  nodes_.clear();
  candidate_nodes_.~priority_queue();
}

void pExtSamp3VTTree::creat_root_node( pExtSamp3VTData* _data )
{
  nodes_.push_back(pExtSamp3VTNode(0,K_));
  pExtSamp3VTNode* root = &(nodes_.back());
...
bool pExtSamp3VTTree::find_best_candidate_split( pExtSamp3VTNode* _node, pExtSamp3VTData* _data )
{
  ...
  pExt3VT_best_split_finder bsf(this,_node,_data);
...
bool pExtSamp3VTTree::find_best_split_num_var( 
  pExtSamp3VTNode* _node, pExtSamp3VTData* _data, int _ivar, pExtSamp3VTSplit &cb_split)
{
  ...
  // initialize
  pExtSamp3VTSolver sol_left(_data, &(this->sub_ci_)), sol_right = _node->sol_this_;
...
void pExtSamp3VTTree::make_node_sorted_idx( pExtSamp3VTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pExtSamp3VTTree::set_best_split_num_var( 
  pExtSamp3VTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
...
bool pExtSamp3VTTree::can_split_node( pExtSamp3VTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pExtSamp3VTTree::split_node( pExtSamp3VTNode* _node, pExtSamp3VTData* _data )
{
  // create left and right node
  pExtSamp3VTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  pExtSamp3VTNode tmp2(nodes_.size(), K_);
...
void pExtSamp3VTTree::calc_gain(pExtSamp3VTNode* _node, pExtSamp3VTData* _data)
{
  double gain;
...
void pExtSamp3VTTree::fit_node( pExtSamp3VTNode* _node, pExtSamp3VTData* _data )
{
  int nn = _node->sample_idx_.size();
...
// Implementation of pExtSamp3VTLogitBoost::Param
pExtSamp3VTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pExtSamp3VTLogitBoost
const double pExtSamp3VTLogitBoost::EPS_LOSS = 1e-14;
const double pExtSamp3VTLogitBoost::MAX_F = 100;
void pExtSamp3VTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pExtSamp3VTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pExtSamp3VTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pExtSamp3VTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pExtSamp3VTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pExtSamp3VTLogitBoost::get_class_count()
{
  return K_;
}

int pExtSamp3VTLogitBoost::get_num_iter()
{
  return NumIter_;
}

//double pExtSamp3VTLogitBoost::get_train_loss()
//{
//  if (NumIter_<1) return DBL_MAX;
//  return L_iter_.at<double>(NumIter_-1);
//}

void pExtSamp3VTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pExtSamp3VTLogitBoost::update_F(int t)
{
...
void pExtSamp3VTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
//bool pExtSamp3VTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void pExtSamp3VTLogitBoost::calc_loss( MLData* _data )
{
...
void pExtSamp3VTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pExtSamp3VTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
...
void pExtSamp3VTLogitBoost::update_weight()
{
  int N = p_.rows;
