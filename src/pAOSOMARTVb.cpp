...
// Implementation of pAOSOMARTSplit
pAOSOMARTSplit::pAOSOMARTSplit()
{
  reset();
}

void pAOSOMARTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pAOSOMARTSolver
//const double pAOSOMARTSolver::EPS = 0.01;
const double pAOSOMARTSolver::MAXGAMMA = 5.0;
pAOSOMARTSolver::pAOSOMARTSolver( pAOSOMARTData* _data, VecIdx* _ci)
{ 
  set_data(_data, _ci);
}

void pAOSOMARTSolver::set_data( pAOSOMARTData* _data, VecIdx* _ci)
{ 
  data_ = _data;
  ci_ = _ci;
...
void pAOSOMARTSolver::update_internal( VecIdx& vidx )
{
...
void pAOSOMARTSolver::update_internal_incre( int idx )
{
...
void pAOSOMARTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pAOSOMARTSolver::calc_gamma( double *gamma)
{
...
void pAOSOMARTSolver::calc_gain( double& gain )
{
  // use gradient least square fitting!
  double mg1 = mg_[0], mg2 = mg_[1];
  double h1 = hu_[0], h2 = hu_[1];

  double mg = mg1 - mg2;
  double h  = h1 + h2 + 2*hhu_;
  if (h==0) h = 1;
...
pAOSOMARTNode::pAOSOMARTNode(int _K)
{
  id_ = 0;
...
pAOSOMARTNode::pAOSOMARTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pAOSOMARTNode::calc_dir( float* _psample )
{
...
// Implementation of pAOSOMART_best_split_finder (helper class)
pAOSOMART_best_split_finder::pAOSOMART_best_split_finder(pAOSOMARTTree *_tree, 
  pAOSOMARTNode *_node, pAOSOMARTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
pAOSOMART_best_split_finder::pAOSOMART_best_split_finder (const pAOSOMART_best_split_finder &f, cv::Split)
{
  this->tree_ = f.tree_;
  this->node_ = f.node_;
  this->data_ = f.data_;
...
void pAOSOMART_best_split_finder::operator() (const cv::BlockedRange &r)
{
...
void pAOSOMART_best_split_finder::join (pAOSOMART_best_split_finder &rhs)
{
...
// Implementation of pAOSOMARTTree::Param
pAOSOMARTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
...
// Implementation of pAOSOMARTTree
void pAOSOMARTTree::split( pAOSOMARTData* _data )
{
  // clear
  clear();
  ...
  pAOSOMARTNode* root = candidate_nodes_.top(); 
  ...
  {
    pAOSOMARTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
pAOSOMARTNode* pAOSOMARTTree::get_node( float* _sample)
{
  pAOSOMARTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pAOSOMARTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}
...
void pAOSOMARTTree::get_is_leaf( VecInt& is_leaf )
{
  is_leaf.clear();
  is_leaf.resize(nodes_.size());

  std::list<pAOSOMARTNode>::iterator it = nodes_.begin();
...
void pAOSOMARTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pAOSOMARTTree::predict( float* _sample, float* _score )
{
  ...
  pAOSOMARTNode* nd = get_node(_sample);
  for (int k = 0; k < K_; ++k) {
...
void pAOSOMARTTree::subsample_samples(pAOSOMARTData* _data)
{
...
void pAOSOMARTTree::subsample_classes_for_node( pAOSOMARTNode* _node, pAOSOMARTData* _data )
{
...
void pAOSOMARTTree::creat_root_node( pAOSOMARTData* _data )
{
  nodes_.push_back(pAOSOMARTNode(0,K_));
  pAOSOMARTNode* root = &(nodes_.back());
...
bool pAOSOMARTTree::find_best_candidate_split( pAOSOMARTNode* _node, pAOSOMARTData* _data )
{
  ...
  pAOSOMART_best_split_finder bsf(this,_node,_data);
...
bool pAOSOMARTTree::find_best_split_num_var( 
  pAOSOMARTNode* _node, pAOSOMARTData* _data, int _ivar, pAOSOMARTSplit &cb_split)
{
  ...
  // initialize
  pAOSOMARTSolver sol_left(_data, _node->sol_this_.ci_), 
...
void pAOSOMARTTree::make_node_sorted_idx( pAOSOMARTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pAOSOMARTTree::set_best_split_num_var( 
  pAOSOMARTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
  int best_i, double best_gain, double best_gain_left, double best_gain_right,
  pAOSOMARTSplit &cb_split)
{
...
bool pAOSOMARTTree::can_split_node( pAOSOMARTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pAOSOMARTTree::split_node( pAOSOMARTNode* _node, pAOSOMARTData* _data )
{
  // create left and right node
  pAOSOMARTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  pAOSOMARTNode tmp2(nodes_.size(), K_);
...
void pAOSOMARTTree::calc_gain(pAOSOMARTNode* _node, pAOSOMARTData* _data)
{
  double gain;
...
void pAOSOMARTTree::fit_node( pAOSOMARTNode* _node, pAOSOMARTData* _data )
{
  int nn = _node->sample_idx_.size();
  if (nn<=0) return;
...
// Implementation of pAOSOMARTVb::Param
pAOSOMARTVb::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pAOSOMARTVb
const double pAOSOMARTVb::EPS_LOSS = 1e-6;
const double pAOSOMARTVb::MAX_F = 100;
void pAOSOMARTVb::train( MLData* _data )
{
  train_init(_data);
...
void pAOSOMARTVb::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pAOSOMARTVb::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pAOSOMARTVb::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pAOSOMARTVb::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pAOSOMARTVb::get_class_count()
{
  return K_;
}

int pAOSOMARTVb::get_num_iter()
{
  return NumIter_;
}

void pAOSOMARTVb::get_nr( VecIdx& nr_wts, VecIdx& nr_wtc )
{
...
void pAOSOMARTVb::get_cc( int itree, VecInt& node_cc )
{
...
void pAOSOMARTVb::get_sc( int itree, VecInt& node_sc )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
void pAOSOMARTVb::get_is_leaf( int itree, VecInt& is_leaf )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
void pAOSOMARTVb::get_si_to_leaf( int itree, VecInt &si_to_leaf )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
void pAOSOMARTVb::get_pp( int itree, MatDbl &p )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
void pAOSOMARTVb::train_init( MLData* _data )
{
  // class count
...
void pAOSOMARTVb::update_F(int t)
{
...
void pAOSOMARTVb::update_abs_grad_class(int t)
{
  int N = F_.rows;
...
void pAOSOMARTVb::calc_loss( MLData* _data )
{
...
void pAOSOMARTVb::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pAOSOMARTVb::calc_loss_class(MLData* _data,  int t )
{
  const double PMIN = 0.0001;
  int N = _data->X.rows;
  for (int i = 0; i < N; ++i) {
...
bool pAOSOMARTVb::should_stop( int t )
{
...
void pAOSOMARTVb::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
