#include "pVbExtSamp11VTLogitBoost.hpp"
#include <functional>
#include <numeric>
...
// Implementation of pVbExtSamp11VTSplit
pVbExtSamp11VTSplit::pVbExtSamp11VTSplit()
{
  reset();
}

void pVbExtSamp11VTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pVbExtSamp11VTSolver
//const double pVbExtSamp11VTSolver::EPS = 0.01;
const double pVbExtSamp11VTSolver::MAXGAMMA = 5.0;
pVbExtSamp11VTSolver::pVbExtSamp11VTSolver( pVbExtSamp11VTData* _data, VecIdx* _ci)
{ 
  set_data(_data, _ci);
}

void pVbExtSamp11VTSolver::set_data( pVbExtSamp11VTData* _data, VecIdx* _ci)
{ 
  data_ = _data;
  ci_ = _ci;
...
void pVbExtSamp11VTSolver::update_internal( VecIdx& vidx )
{
...
void pVbExtSamp11VTSolver::update_internal_incre( int idx )
{
...
void pVbExtSamp11VTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pVbExtSamp11VTSolver::calc_gamma( double *gamma)
{
  int KK = mg_.size();
  for (int kk = 0; kk < KK; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
    //if (sh <= 0) cv::error("pVbExtSamp11VTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void pVbExtSamp11VTSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
// Implementation of pVbExtSamp11VTNode
pVbExtSamp11VTNode::pVbExtSamp11VTNode(int _K)
{
  id_ = 0;
...
pVbExtSamp11VTNode::pVbExtSamp11VTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pVbExtSamp11VTNode::calc_dir( float* _psample )
{
...
// Implementation of pVbExt11VT_best_split_finder (helper class)
pVbExt11VT_best_split_finder::pVbExt11VT_best_split_finder(pVbExtSamp11VTTree *_tree, 
  pVbExtSamp11VTNode *_node, pVbExtSamp11VTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
pVbExt11VT_best_split_finder::pVbExt11VT_best_split_finder (const pVbExt11VT_best_split_finder &f, cv::Split)
{
  this->tree_ = f.tree_;
  this->node_ = f.node_;
  this->data_ = f.data_;
...
void pVbExt11VT_best_split_finder::operator() (const cv::BlockedRange &r)
{
...
void pVbExt11VT_best_split_finder::join (pVbExt11VT_best_split_finder &rhs)
{
...
// Implementation of pVbExtSamp11VTTree::Param
pVbExtSamp11VTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
...
// Implementation of pVbExtSamp11VTTree
void pVbExtSamp11VTTree::split( pVbExtSamp11VTData* _data )
{
  // clear
  clear();
  ...
  pVbExtSamp11VTNode* root = candidate_nodes_.top(); 
  ...
  {
    pVbExtSamp11VTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void pVbExtSamp11VTTree::fit( pVbExtSamp11VTData* _data )
{
  // fitting node data for each leaf
  std::list<pVbExtSamp11VTNode>::iterator it;
  for (it = nodes_.begin(); it != nodes_.end(); ++it) {
    pVbExtSamp11VTNode* nd = &(*it);
...
pVbExtSamp11VTNode* pVbExtSamp11VTTree::get_node( float* _sample)
{
  pVbExtSamp11VTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pVbExtSamp11VTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}

void pVbExtSamp11VTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pVbExtSamp11VTTree::predict( float* _sample, float* _score )
{
  ...
  pVbExtSamp11VTNode* nd = get_node(_sample);
  for (int k = 0; k < K_; ++k) {
...
void pVbExtSamp11VTTree::subsample(pVbExtSamp11VTData* _data)
{
  /// subsample samples
  // sample wise
...
void pVbExtSamp11VTTree::clear()
{
  nodes_.clear();
  candidate_nodes_.~priority_queue();
}

void pVbExtSamp11VTTree::creat_root_node( pVbExtSamp11VTData* _data )
{
  nodes_.push_back(pVbExtSamp11VTNode(0,K_));
  pVbExtSamp11VTNode* root = &(nodes_.back());
...
bool pVbExtSamp11VTTree::find_best_candidate_split( pVbExtSamp11VTNode* _node, pVbExtSamp11VTData* _data )
{
  ...
  pVbExt11VT_best_split_finder bsf(this,_node,_data);
...
bool pVbExtSamp11VTTree::find_best_split_num_var( 
  pVbExtSamp11VTNode* _node, pVbExtSamp11VTData* _data, int _ivar, pVbExtSamp11VTSplit &cb_split)
{
  ...
  // initialize
  pVbExtSamp11VTSolver sol_left(_data, _node->sol_this_.ci_), 
...
void pVbExtSamp11VTTree::make_node_sorted_idx( pVbExtSamp11VTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pVbExtSamp11VTTree::set_best_split_num_var( 
  pVbExtSamp11VTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
...
bool pVbExtSamp11VTTree::can_split_node( pVbExtSamp11VTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pVbExtSamp11VTTree::split_node( pVbExtSamp11VTNode* _node, pVbExtSamp11VTData* _data )
{
  // create left and right node
  pVbExtSamp11VTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  pVbExtSamp11VTNode tmp2(nodes_.size(), K_);
...
void pVbExtSamp11VTTree::calc_gain(pVbExtSamp11VTNode* _node, pVbExtSamp11VTData* _data)
{
  double gain;
...
void pVbExtSamp11VTTree::fit_node( pVbExtSamp11VTNode* _node, pVbExtSamp11VTData* _data )
{
  int nn = _node->sample_idx_.size();
  if (nn<=0) return;
  ...
  for (int k = 0; k < this->K_; ++k)
    ci[k] = k;
  pVbExtSamp11VTSolver sol(_data, &ci);
...
// Implementation of pVbExtSamp11VTLogitBoost::Param
pVbExtSamp11VTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pVbExtSamp11VTLogitBoost
const double pVbExtSamp11VTLogitBoost::EPS_LOSS = 1e-14;
const double pVbExtSamp11VTLogitBoost::MAX_F = 100;
void pVbExtSamp11VTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pVbExtSamp11VTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pVbExtSamp11VTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pVbExtSamp11VTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pVbExtSamp11VTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pVbExtSamp11VTLogitBoost::get_class_count()
{
  return K_;
}

int pVbExtSamp11VTLogitBoost::get_num_iter()
{
  return NumIter_;
}

void pVbExtSamp11VTLogitBoost::get_nr( VecIdx& nr_wts, VecIdx& nr_wtc )
{
...
void pVbExtSamp11VTLogitBoost::get_cc( int itree, VecInt& node_cc )
{
...
void pVbExtSamp11VTLogitBoost::get_sc( int itree, VecInt& node_sc )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
//double pVbExtSamp11VTLogitBoost::get_train_loss()
//{
//  if (NumIter_<1) return DBL_MAX;
//  return L_iter_.at<double>(NumIter_-1);
//}

void pVbExtSamp11VTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pVbExtSamp11VTLogitBoost::update_F(int t)
{
...
void pVbExtSamp11VTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
void pVbExtSamp11VTLogitBoost::update_gg()
{
  int N = F_.rows;
  double delta = 0;
...
//bool pVbExtSamp11VTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void pVbExtSamp11VTLogitBoost::calc_loss( MLData* _data )
{
...
void pVbExtSamp11VTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pVbExtSamp11VTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
