#include "pVbExtSamp7VTLogitBoost.hpp"
#include <functional>
#include <numeric>
...
namespace {
  ...
  void calc_w_1norm_samp (MatDbl& ww, MatDbl& out) {
    int nrow = ww.rows;
    out.create(nrow,1);
  ...
  void calc_w_1norm_class (MatDbl& ww, MatDbl& out) {
    int ncol = ww.cols;
    out.create(ncol,1);
...
// Implementation of pVbExtSamp7VTSplit
pVbExtSamp7VTSplit::pVbExtSamp7VTSplit()
{
  reset();
}

void pVbExtSamp7VTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pVbExtSamp7VTSolver
//const double pVbExtSamp7VTSolver::EPS = 0.01;
const double pVbExtSamp7VTSolver::MAXGAMMA = 5.0;
pVbExtSamp7VTSolver::pVbExtSamp7VTSolver( pVbExtSamp7VTData* _data, VecIdx* _ci)
{ 
  set_data(_data, _ci);
}

void pVbExtSamp7VTSolver::set_data( pVbExtSamp7VTData* _data, VecIdx* _ci)
{ 
  data_ = _data;
  ci_ = _ci;
...
void pVbExtSamp7VTSolver::update_internal( VecIdx& vidx )
{
...
void pVbExtSamp7VTSolver::update_internal_incre( int idx )
{
...
void pVbExtSamp7VTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pVbExtSamp7VTSolver::calc_gamma( double *gamma)
{
  int KK = mg_.size();
  for (int kk = 0; kk < KK; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
    //if (sh <= 0) cv::error("pVbExtSamp7VTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void pVbExtSamp7VTSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
// Implementation of pVbExtSamp7VTNode
pVbExtSamp7VTNode::pVbExtSamp7VTNode(int _K)
{
  id_ = 0;
...
pVbExtSamp7VTNode::pVbExtSamp7VTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pVbExtSamp7VTNode::calc_dir( float* _psample )
{
...
// Implementation of pVbExt5VT_best_split_finder (helper class)
pVbExt5VT_best_split_finder::pVbExt5VT_best_split_finder(pVbExtSamp7VTTree *_tree, 
  pVbExtSamp7VTNode *_node, pVbExtSamp7VTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
pVbExt5VT_best_split_finder::pVbExt5VT_best_split_finder (const pVbExt5VT_best_split_finder &f, cv::Split)
{
  this->tree_ = f.tree_;
  this->node_ = f.node_;
  this->data_ = f.data_;
...
void pVbExt5VT_best_split_finder::operator() (const cv::BlockedRange &r)
{
...
void pVbExt5VT_best_split_finder::join (pVbExt5VT_best_split_finder &rhs)
{
...
// Implementation of pVbExtSamp7VTTree::Param
pVbExtSamp7VTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
...
// Implementation of pVbExtSamp7VTTree
void pVbExtSamp7VTTree::split( pVbExtSamp7VTData* _data )
{
  // clear
  clear();
  ...
  pVbExtSamp7VTNode* root = candidate_nodes_.top(); 
  ...
  {
    pVbExtSamp7VTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void pVbExtSamp7VTTree::fit( pVbExtSamp7VTData* _data )
{
  // fitting node data for each leaf
  std::list<pVbExtSamp7VTNode>::iterator it;
  for (it = nodes_.begin(); it != nodes_.end(); ++it) {
    pVbExtSamp7VTNode* nd = &(*it);
...
pVbExtSamp7VTNode* pVbExtSamp7VTTree::get_node( float* _sample)
{
  pVbExtSamp7VTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pVbExtSamp7VTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}

void pVbExtSamp7VTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pVbExtSamp7VTTree::predict( float* _sample, float* _score )
{
  ...
  pVbExtSamp7VTNode* nd = get_node(_sample);
  for (int k = 0; k < K_; ++k) {
...
void pVbExtSamp7VTTree::subsample(pVbExtSamp7VTData* _data)
{
  /// subsample samples
  // sample wise
  cv::Mat_<double> w_samp;
  calc_w_1norm_samp( *_data->ww_, w_samp); // after: N * 1
  // 
  VecIdx si_wt;
  weight_trim_samp_ratio(w_samp, this->param_.ratio_si_, si_wt);
  // record & set it
  ...
  MatDbl tmp;
  subsample_rows(*_data->ww_, this->sub_si_, tmp);
  cv::Mat_<double> w_class; 
  calc_w_1norm_class( tmp, w_class); // after: K * 1
  // 
  //// class wise
  //cv::Mat_<double> w_class; 
  //calc_1norm_proj_to_col( *_data->ww_, w_class); // after: K * 1
  //// 
  VecIdx ci_wt;
  weight_trim_samp_ratio(w_class, this->param_.ratio_ci_, ci_wt);
  // record & set it
...
void pVbExtSamp7VTTree::clear()
{
  nodes_.clear();
  candidate_nodes_.~priority_queue();
}

void pVbExtSamp7VTTree::creat_root_node( pVbExtSamp7VTData* _data )
{
  nodes_.push_back(pVbExtSamp7VTNode(0,K_));
  pVbExtSamp7VTNode* root = &(nodes_.back());
...
bool pVbExtSamp7VTTree::find_best_candidate_split( pVbExtSamp7VTNode* _node, pVbExtSamp7VTData* _data )
{
  ...
  pVbExt5VT_best_split_finder bsf(this,_node,_data);
...
bool pVbExtSamp7VTTree::find_best_split_num_var( 
  pVbExtSamp7VTNode* _node, pVbExtSamp7VTData* _data, int _ivar, pVbExtSamp7VTSplit &cb_split)
{
  ...
  // initialize
  pVbExtSamp7VTSolver sol_left(_data, &(this->sub_ci_)), sol_right = _node->sol_this_;
...
void pVbExtSamp7VTTree::make_node_sorted_idx( pVbExtSamp7VTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pVbExtSamp7VTTree::set_best_split_num_var( 
  pVbExtSamp7VTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
...
bool pVbExtSamp7VTTree::can_split_node( pVbExtSamp7VTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pVbExtSamp7VTTree::split_node( pVbExtSamp7VTNode* _node, pVbExtSamp7VTData* _data )
{
  // create left and right node
  pVbExtSamp7VTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  pVbExtSamp7VTNode tmp2(nodes_.size(), K_);
...
void pVbExtSamp7VTTree::calc_gain(pVbExtSamp7VTNode* _node, pVbExtSamp7VTData* _data)
{
  double gain;
...
void pVbExtSamp7VTTree::fit_node( pVbExtSamp7VTNode* _node, pVbExtSamp7VTData* _data )
{
  int nn = _node->sample_idx_.size();
...
// Implementation of pVbExtSamp7VTLogitBoost::Param
pVbExtSamp7VTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pVbExtSamp7VTLogitBoost
const double pVbExtSamp7VTLogitBoost::EPS_LOSS = 1e-14;
const double pVbExtSamp7VTLogitBoost::MAX_F = 100;
void pVbExtSamp7VTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pVbExtSamp7VTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pVbExtSamp7VTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pVbExtSamp7VTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pVbExtSamp7VTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pVbExtSamp7VTLogitBoost::get_class_count()
{
  return K_;
}

int pVbExtSamp7VTLogitBoost::get_num_iter()
{
  return NumIter_;
}

void pVbExtSamp7VTLogitBoost::get_nr( VecIdx& nr_wts, VecIdx& nr_wtc )
{
...
//double pVbExtSamp7VTLogitBoost::get_train_loss()
//{
//  if (NumIter_<1) return DBL_MAX;
//  return L_iter_.at<double>(NumIter_-1);
//}

void pVbExtSamp7VTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pVbExtSamp7VTLogitBoost::update_F(int t)
{
...
void pVbExtSamp7VTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
void pVbExtSamp7VTLogitBoost::update_ww()
{
  int N = F_.rows;
  double delta = 0;
...
//bool pVbExtSamp7VTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void pVbExtSamp7VTLogitBoost::calc_loss( MLData* _data )
{
...
void pVbExtSamp7VTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pVbExtSamp7VTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
