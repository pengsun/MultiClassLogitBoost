#include "pExtSampVTLogitBoost.hpp"
#include <functional>
...
// Implementation of pExtSampVTSplit
pExtSampVTSplit::pExtSampVTSplit()
{
  reset();
}

void pExtSampVTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pExtSampVTSolver
//const double pExtSampVTSolver::EPS = 0.01;
const double pExtSampVTSolver::MAXGAMMA = 5.0;
pExtSampVTSolver::pExtSampVTSolver( pExtSampVTData* _data, VecIdx* _ci)
{ 
  set_data(_data, _ci);
}

void pExtSampVTSolver::set_data( pExtSampVTData* _data, VecIdx* _ci)
{ 
  data_ = _data;
  ci_ = _ci;
...
void pExtSampVTSolver::update_internal( VecIdx& vidx )
{
...
void pExtSampVTSolver::update_internal_incre( int idx )
{
...
void pExtSampVTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pExtSampVTSolver::calc_gamma( double *gamma)
{
  int KK = mg_.size();
  for (int kk = 0; kk < KK; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
    //if (sh <= 0) cv::error("pExtSampVTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void pExtSampVTSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
// Implementation of pExtSampVTNode
pExtSampVTNode::pExtSampVTNode(int _K)
{
  id_ = 0;
...
pExtSampVTNode::pExtSampVTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pExtSampVTNode::calc_dir( float* _psample )
{
...
// Implementation of pExtVT_best_split_finder (helper class)
pExtVT_best_split_finder::pExtVT_best_split_finder(pExtSampVTTree *_tree, 
  pExtSampVTNode *_node, pExtSampVTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
pExtVT_best_split_finder::pExtVT_best_split_finder (const pExtVT_best_split_finder &f, cv::Split)
{
  this->tree_ = f.tree_;
  this->node_ = f.node_;
  this->data_ = f.data_;
...
void pExtVT_best_split_finder::operator() (const cv::BlockedRange &r)
{
...
void pExtVT_best_split_finder::join (pExtVT_best_split_finder &rhs)
{
...
// Implementation of pExtSampVTTree::Param
pExtSampVTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
...
// Implementation of pExtSampVTTree
void pExtSampVTTree::split( pExtSampVTData* _data )
{
  // clear
  clear();
  ...
  pExtSampVTNode* root = candidate_nodes_.top(); 
  ...
  {
    pExtSampVTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void pExtSampVTTree::fit( pExtSampVTData* _data )
{
  // fitting node data for each leaf
  std::list<pExtSampVTNode>::iterator it;
  for (it = nodes_.begin(); it != nodes_.end(); ++it) {
    pExtSampVTNode* nd = &(*it);
...
pExtSampVTNode* pExtSampVTTree::get_node( float* _sample)
{
  pExtSampVTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pExtSampVTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}

void pExtSampVTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pExtSampVTTree::predict( float* _sample, float* _score )
{
  ...
  pExtSampVTNode* nd = get_node(_sample);
  for (int k = 0; k < K_; ++k) {
...
void pExtSampVTTree::subsample(pExtSampVTData* _data)
{
  // subsample samples
  ...
  pExtSampVTSolver tmp_sol(_data, &all_ci);
...
void pExtSampVTTree::clear()
{
  nodes_.clear();
  candidate_nodes_.~priority_queue();
}

void pExtSampVTTree::creat_root_node( pExtSampVTData* _data )
{
  nodes_.push_back(pExtSampVTNode(0,K_));
  pExtSampVTNode* root = &(nodes_.back());
...
bool pExtSampVTTree::find_best_candidate_split( pExtSampVTNode* _node, pExtSampVTData* _data )
{
  ...
  pExtVT_best_split_finder bsf(this,_node,_data);
...
bool pExtSampVTTree::find_best_split_num_var( 
  pExtSampVTNode* _node, pExtSampVTData* _data, int _ivar, pExtSampVTSplit &cb_split)
{
  ...
  // initialize
  pExtSampVTSolver sol_left(_data, &(this->sub_ci_)), sol_right = _node->sol_this_;
...
void pExtSampVTTree::make_node_sorted_idx( pExtSampVTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pExtSampVTTree::set_best_split_num_var( 
  pExtSampVTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
...
bool pExtSampVTTree::can_split_node( pExtSampVTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pExtSampVTTree::split_node( pExtSampVTNode* _node, pExtSampVTData* _data )
{
  // create left and right node
  pExtSampVTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  pExtSampVTNode tmp2(nodes_.size(), K_);
...
void pExtSampVTTree::calc_gain(pExtSampVTNode* _node, pExtSampVTData* _data)
{
  double gain;
...
void pExtSampVTTree::fit_node( pExtSampVTNode* _node, pExtSampVTData* _data )
{
  int nn = _node->sample_idx_.size();
...
// Implementation of pExtSampVTLogitBoost::Param
pExtSampVTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pExtSampVTLogitBoost
const double pExtSampVTLogitBoost::EPS_LOSS = 1e-14;
const double pExtSampVTLogitBoost::MAX_F = 100;
void pExtSampVTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pExtSampVTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pExtSampVTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pExtSampVTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pExtSampVTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pExtSampVTLogitBoost::get_class_count()
{
  return K_;
}

int pExtSampVTLogitBoost::get_num_iter()
{
  return NumIter_;
}

//double pExtSampVTLogitBoost::get_train_loss()
//{
//  if (NumIter_<1) return DBL_MAX;
//  return L_iter_.at<double>(NumIter_-1);
//}

void pExtSampVTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pExtSampVTLogitBoost::update_F(int t)
{
...
void pExtSampVTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
//bool pExtSampVTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void pExtSampVTLogitBoost::calc_loss( MLData* _data )
{
...
void pExtSampVTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pExtSampVTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
