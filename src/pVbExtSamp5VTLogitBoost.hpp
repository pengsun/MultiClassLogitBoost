...
// Solver
struct pVbExtSamp5VTSolver {
  static const double MAXGAMMA;
  //static const double EPS;

  pVbExtSamp5VTSolver () {};
  pVbExtSamp5VTSolver (pVbExtSamp5VTData* _data, VecIdx* _ci);
  void set_data (pVbExtSamp5VTData* _data, VecIdx* _ci);
...
public:
  ...
  pVbExtSamp5VTData* data_;
  VecIdx *ci_;
};

// Split descriptor
struct pVbExtSamp5VTSplit {
public:
  pVbExtSamp5VTSplit ();
  void reset ();
...
struct pVbExtSamp5VTNode {
public:
  pVbExtSamp5VTNode (int _K);
  pVbExtSamp5VTNode (int _id, int _K);
...
public:
  ...
  pVbExtSamp5VTNode *parent_, *left_, *right_; //
  pVbExtSamp5VTSplit split_;

  VecIdx sample_idx_; // for all the examples this node holds

  pVbExtSamp5VTSolver sol_this_; // for all the examples this node holds
};

// Node Comparator: the less the expected gain, the less the node
struct pVbExtSamp5VTNodeLess {
  bool operator () (const pVbExtSamp5VTNode* n1, const pVbExtSamp5VTNode* n2) {
...
typedef std::priority_queue<pVbExtSamp5VTNode*, 
                            std::vector<pVbExtSamp5VTNode*>, 
                            pVbExtSamp5VTNodeLess>
        QuepVbExtSamp5VTNode;

// Best Split Finder (helper class for parallel_reduce) 
class pVbExtSamp5VTTree;
struct pVbExt5VT_best_split_finder {
  pVbExt5VT_best_split_finder (pVbExtSamp5VTTree *_tree, 
    pVbExtSamp5VTNode* _node, pVbExtSamp5VTData* _data); // customized data
  ...
  pVbExtSamp5VTTree *tree_;
  pVbExtSamp5VTNode *node_;
  pVbExtSamp5VTData *data_;
  pVbExtSamp5VTSplit cb_split_;
};
...
public:
  void split( pVbExtSamp5VTData* _data );
  void fit ( pVbExtSamp5VTData* _data );

  pVbExtSamp5VTNode* get_node (float* _sample);
  void predict (MLData* _data);
  void predict (float* _sample, float* _score);

public:
  void subsample (pVbExtSamp5VTData* _data);

  void clear ();
  void creat_root_node (pVbExtSamp5VTData* _data);

  virtual bool find_best_candidate_split (pVbExtSamp5VTNode* _node, pVbExtSamp5VTData* _data);
  virtual bool find_best_split_num_var (pVbExtSamp5VTNode* _node, pVbExtSamp5VTData* _data, int _ivar,
                                        pVbExtSamp5VTSplit &spl);

  void make_node_sorted_idx(pVbExtSamp5VTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node);
  bool set_best_split_num_var ( pVbExtSamp5VTNode* _node, MLData* _data, int _ivar, 
    VecIdx& node_sample_si,
    int best_i, double best_gain, double best_gain_left, double best_gain_right,
    pVbExtSamp5VTSplit &cb_split);

  bool can_split_node (pVbExtSamp5VTNode* _node);
  bool split_node (pVbExtSamp5VTNode* _node, pVbExtSamp5VTData* _data);
  void calc_gain (pVbExtSamp5VTNode* _node, pVbExtSamp5VTData* _data);
  virtual void fit_node (pVbExtSamp5VTNode* _node, pVbExtSamp5VTData* _data);

protected:
  std::list<pVbExtSamp5VTNode> nodes_; // all nodes
  QuepVbExtSamp5VTNode candidate_nodes_; // priority queue of candidate leaves for splitting
  // cb: current best
...
// vector of pVbExtSamp5VTTree
typedef std::vector<pVbExtSamp5VTTree> VecpVbExtSamp5VTTree;

// Boost
class pVbExtSamp5VTLogitBoost {
public:
...
protected:
  ...
  pVbExtSamp5VTData logitdata_;
  VecpVbExtSamp5VTTree trees_;
