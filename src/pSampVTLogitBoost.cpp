...
pSampVTSplit::pSampVTSplit()
{
  reset();
}

void pSampVTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pSampVTSolver
//const double pSampVTSolver::EPS = 0.01;
const double pSampVTSolver::MAXGAMMA = 5.0;
pSampVTSolver::pSampVTSolver( pSampVTData* _data, VecIdx* _ci)
{ 
  set_data(_data, _ci);
}

void pSampVTSolver::set_data( pSampVTData* _data, VecIdx* _ci)
{ 
  data_ = _data;
  ci_ = _ci;
...
void pSampVTSolver::update_internal( VecIdx& vidx )
{
...
void pSampVTSolver::update_internal_incre( int idx )
{
...
void pSampVTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pSampVTSolver::calc_gamma( double *gamma)
{
  int KK = mg_.size();
  for (int kk = 0; kk < KK; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
    //if (sh <= 0) cv::error("pSampVTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void pSampVTSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
// Implementation of pSampVTNode
pSampVTNode::pSampVTNode(int _K)
{
  id_ = 0;
...
pSampVTNode::pSampVTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pSampVTNode::calc_dir( float* _psample )
{
...
pVT_best_split_finder::pVT_best_split_finder(pSampVTTree *_tree, 
  pSampVTNode *_node, pSampVTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
// Implementation of pSampVTTree::Param
pSampVTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
...
// Implementation of pSampVTTree
void pSampVTTree::split( pSampVTData* _data )
{
  // clear
  clear();
  ...
  pSampVTNode* root = candidate_nodes_.top(); 
  ...
  {
    pSampVTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void pSampVTTree::fit( pSampVTData* _data )
{
  // fitting node data for each leaf
  std::list<pSampVTNode>::iterator it;
...
pSampVTNode* pSampVTTree::get_node( float* _sample)
{
  pSampVTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pSampVTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}

void pSampVTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pSampVTTree::predict( float* _sample, float* _score )
{
...
void pSampVTTree::subsample(pSampVTData* _data)
{
  // subsample samples
...
void pSampVTTree::creat_root_node( pSampVTData* _data )
{
  nodes_.push_back(pSampVTNode(0,K_));
  pSampVTNode* root = &(nodes_.back());
...
bool pSampVTTree::find_best_candidate_split( pSampVTNode* _node, pSampVTData* _data )
{
...
bool pSampVTTree::find_best_split_num_var( 
  pSampVTNode* _node, pSampVTData* _data, int _ivar, pSampVTSplit &cb_split)
{
  ...
  // initialize
  pSampVTSolver sol_left(_data, &(this->sub_ci_)), sol_right = _node->sol_this_;
...
void pSampVTTree::make_node_sorted_idx( pSampVTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pSampVTTree::set_best_split_num_var( 
  pSampVTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
...
bool pSampVTTree::can_split_node( pSampVTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pSampVTTree::split_node( pSampVTNode* _node, pSampVTData* _data )
{
  // create left and right node
  pSampVTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  pSampVTNode tmp2(nodes_.size(), K_);
...
void pSampVTTree::calc_gain(pSampVTNode* _node, pSampVTData* _data)
{
  double gain;
...
// Implementation of pSampVTLogitBoost::Param
pSampVTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pSampVTLogitBoost
const double pSampVTLogitBoost::EPS_LOSS = 1e-14;
const double pSampVTLogitBoost::MAX_F = 100;
void pSampVTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pSampVTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pSampVTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pSampVTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pSampVTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pSampVTLogitBoost::get_class_count()
{
  return K_;
}

int pSampVTLogitBoost::get_num_iter()
{
  return NumIter_;
}

//double pSampVTLogitBoost::get_train_loss()
//{
//  if (NumIter_<1) return DBL_MAX;
//  return L_iter_.at<double>(NumIter_-1);
//}

void pSampVTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pSampVTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
//bool pSampVTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void pSampVTLogitBoost::calc_loss( MLData* _data )
{
...
void pSampVTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pSampVTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
