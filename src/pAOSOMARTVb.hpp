...
// Solver
struct pAOSOMARTSolver {
  static const double MAXGAMMA;
  //static const double EPS;

  pAOSOMARTSolver () {};
  pAOSOMARTSolver (pAOSOMARTData* _data, VecIdx* _ci);
  void set_data (pAOSOMARTData* _data, VecIdx* _ci);
...
struct pAOSOMARTNode {
public:
  pAOSOMARTNode (int _K);
  pAOSOMARTNode (int _id, int _K);
...
public:
  ...
  pAOSOMARTNode *parent_, *left_, *right_; //
  pAOSOMARTSplit split_;

  VecIdx sample_idx_; // for all the examples this node holds
  VecIdx sub_ci_; // subclass set
  pAOSOMARTSolver sol_this_; // for all the examples this node holds
};

// Node Comparator: the less the expected gain, the less the node
struct pAOSOMARTNodeLess {
  bool operator () (const pAOSOMARTNode* n1, const pAOSOMARTNode* n2) {
...
typedef std::priority_queue<pAOSOMARTNode*, 
                            std::vector<pAOSOMARTNode*>, 
                            pAOSOMARTNodeLess>
        QuepAOSOMARTNode;

// Best Split Finder (helper class for parallel_reduce) 
class pAOSOMARTTree;
struct pAOSOMART_best_split_finder {
  pAOSOMART_best_split_finder (pAOSOMARTTree *_tree, 
    pAOSOMARTNode* _node, pAOSOMARTData* _data); // customized data
  pAOSOMART_best_split_finder (const pAOSOMART_best_split_finder &f, cv::Split); // required

  void operator () (const cv::BlockedRange &r); // required
  void join (pAOSOMART_best_split_finder &rhs); // required

  pAOSOMARTTree *tree_;
  pAOSOMARTNode *node_;
  pAOSOMARTData *data_;
  pAOSOMARTSplit cb_split_;
};
...
public:
  void split( pAOSOMARTData* _data );
  void fit ( pAOSOMARTData* _data );

  pAOSOMARTNode* get_node (float* _sample);
  void get_is_leaf (VecInt& is_leaf);
  void predict (MLData* _data);
  void predict (float* _sample, float* _score);

public:
  void subsample_samples (pAOSOMARTData* _data);
  void subsample_classes_for_node (pAOSOMARTNode* _node, pAOSOMARTData* _data);

  void clear ();
  void creat_root_node (pAOSOMARTData* _data);

  virtual bool find_best_candidate_split (pAOSOMARTNode* _node, pAOSOMARTData* _data);
  virtual bool find_best_split_num_var (pAOSOMARTNode* _node, pAOSOMARTData* _data, int _ivar,
    pAOSOMARTSplit &spl);

  void make_node_sorted_idx(pAOSOMARTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node);
  bool set_best_split_num_var ( pAOSOMARTNode* _node, MLData* _data, int _ivar, 
    VecIdx& node_sample_si,
    int best_i, double best_gain, double best_gain_left, double best_gain_right,
    pAOSOMARTSplit &cb_split);

  bool can_split_node (pAOSOMARTNode* _node);
  bool split_node (pAOSOMARTNode* _node, pAOSOMARTData* _data);
  void calc_gain (pAOSOMARTNode* _node, pAOSOMARTData* _data);
  virtual void fit_node (pAOSOMARTNode* _node, pAOSOMARTData* _data);

protected:
  std::list<pAOSOMARTNode> nodes_; // all nodes
  QuepAOSOMARTNode candidate_nodes_; // priority queue of candidate leaves for splitting
  // cb: current best
  // caching internal data, used by find_best_split*
  int K_; // #classes
};

// vector of pAOSOMARTTree
typedef std::vector<pAOSOMARTTree> VecpAOSOMARTTree;
...
protected:
  ...
  pAOSOMARTData logitdata_;
  VecpAOSOMARTTree trees_;
