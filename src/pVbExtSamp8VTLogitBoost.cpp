#include "pVbExtSamp8VTLogitBoost.hpp"
#include <functional>
#include <numeric>
...
// Implementation of pVbExtSamp8VTSplit
pVbExtSamp8VTSplit::pVbExtSamp8VTSplit()
{
  reset();
}

void pVbExtSamp8VTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pVbExtSamp8VTSolver
//const double pVbExtSamp8VTSolver::EPS = 0.01;
const double pVbExtSamp8VTSolver::MAXGAMMA = 5.0;
pVbExtSamp8VTSolver::pVbExtSamp8VTSolver( pVbExtSamp8VTData* _data, VecIdx* _ci)
{ 
  set_data(_data, _ci);
}

void pVbExtSamp8VTSolver::set_data( pVbExtSamp8VTData* _data, VecIdx* _ci)
{ 
  data_ = _data;
  ci_ = _ci;
...
void pVbExtSamp8VTSolver::update_internal( VecIdx& vidx )
{
...
void pVbExtSamp8VTSolver::update_internal_incre( int idx )
{
...
void pVbExtSamp8VTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pVbExtSamp8VTSolver::calc_gamma( double *gamma)
{
  int KK = mg_.size();
  for (int kk = 0; kk < KK; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
    //if (sh <= 0) cv::error("pVbExtSamp8VTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void pVbExtSamp8VTSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
// Implementation of pVbExtSamp8VTNode
pVbExtSamp8VTNode::pVbExtSamp8VTNode(int _K)
{
  id_ = 0;
...
pVbExtSamp8VTNode::pVbExtSamp8VTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pVbExtSamp8VTNode::calc_dir( float* _psample )
{
...
// Implementation of pVbExt8VT_best_split_finder (helper class)
pVbExt8VT_best_split_finder::pVbExt8VT_best_split_finder(pVbExtSamp8VTTree *_tree, 
  pVbExtSamp8VTNode *_node, pVbExtSamp8VTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
pVbExt8VT_best_split_finder::pVbExt8VT_best_split_finder (const pVbExt8VT_best_split_finder &f, cv::Split)
{
  this->tree_ = f.tree_;
  this->node_ = f.node_;
  this->data_ = f.data_;
...
void pVbExt8VT_best_split_finder::operator() (const cv::BlockedRange &r)
{
...
void pVbExt8VT_best_split_finder::join (pVbExt8VT_best_split_finder &rhs)
{
...
// Implementation of pVbExtSamp8VTTree::Param
pVbExtSamp8VTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
...
// Implementation of pVbExtSamp8VTTree
void pVbExtSamp8VTTree::split( pVbExtSamp8VTData* _data )
{
  // clear
  clear();
  ...
  pVbExtSamp8VTNode* root = candidate_nodes_.top(); 
  ...
  {
    pVbExtSamp8VTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void pVbExtSamp8VTTree::fit( pVbExtSamp8VTData* _data )
{
  // fitting node data for each leaf
  std::list<pVbExtSamp8VTNode>::iterator it;
  for (it = nodes_.begin(); it != nodes_.end(); ++it) {
    pVbExtSamp8VTNode* nd = &(*it);
...
pVbExtSamp8VTNode* pVbExtSamp8VTTree::get_node( float* _sample)
{
  pVbExtSamp8VTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pVbExtSamp8VTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}

void pVbExtSamp8VTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pVbExtSamp8VTTree::predict( float* _sample, float* _score )
{
  ...
  pVbExtSamp8VTNode* nd = get_node(_sample);
  for (int k = 0; k < K_; ++k) {
...
void pVbExtSamp8VTTree::subsample(pVbExtSamp8VTData* _data)
{
  /// subsample samples
  // sample wise
...
void pVbExtSamp8VTTree::subsample_classes_for_node( pVbExtSamp8VTNode* _node, pVbExtSamp8VTData* _data )
{
  /// subsample classes
...
void pVbExtSamp8VTTree::clear()
{
  nodes_.clear();
  candidate_nodes_.~priority_queue();
}

void pVbExtSamp8VTTree::creat_root_node( pVbExtSamp8VTData* _data )
{
  nodes_.push_back(pVbExtSamp8VTNode(0,K_));
  pVbExtSamp8VTNode* root = &(nodes_.back());
...
bool pVbExtSamp8VTTree::find_best_candidate_split( pVbExtSamp8VTNode* _node, pVbExtSamp8VTData* _data )
{
  ...
  pVbExt8VT_best_split_finder bsf(this,_node,_data);
...
bool pVbExtSamp8VTTree::find_best_split_num_var( 
  pVbExtSamp8VTNode* _node, pVbExtSamp8VTData* _data, int _ivar, pVbExtSamp8VTSplit &cb_split)
{
  ...
  // initialize
  pVbExtSamp8VTSolver sol_left(_data, _node->sol_this_.ci_), 
...
void pVbExtSamp8VTTree::make_node_sorted_idx( pVbExtSamp8VTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pVbExtSamp8VTTree::set_best_split_num_var( 
  pVbExtSamp8VTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
...
bool pVbExtSamp8VTTree::can_split_node( pVbExtSamp8VTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pVbExtSamp8VTTree::split_node( pVbExtSamp8VTNode* _node, pVbExtSamp8VTData* _data )
{
  // create left and right node
  pVbExtSamp8VTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  pVbExtSamp8VTNode tmp2(nodes_.size(), K_);
...
void pVbExtSamp8VTTree::calc_gain(pVbExtSamp8VTNode* _node, pVbExtSamp8VTData* _data)
{
  double gain;
...
void pVbExtSamp8VTTree::fit_node( pVbExtSamp8VTNode* _node, pVbExtSamp8VTData* _data )
{
  int nn = _node->sample_idx_.size();
...
// Implementation of pVbExtSamp8VTLogitBoost::Param
pVbExtSamp8VTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pVbExtSamp8VTLogitBoost
const double pVbExtSamp8VTLogitBoost::EPS_LOSS = 1e-14;
const double pVbExtSamp8VTLogitBoost::MAX_F = 100;
void pVbExtSamp8VTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pVbExtSamp8VTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pVbExtSamp8VTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pVbExtSamp8VTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pVbExtSamp8VTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pVbExtSamp8VTLogitBoost::get_class_count()
{
  return K_;
}

int pVbExtSamp8VTLogitBoost::get_num_iter()
{
  return NumIter_;
}

void pVbExtSamp8VTLogitBoost::get_nr( VecIdx& nr_wts, VecIdx& nr_wtc )
{
...
//double pVbExtSamp8VTLogitBoost::get_train_loss()
//{
//  if (NumIter_<1) return DBL_MAX;
//  return L_iter_.at<double>(NumIter_-1);
//}

void pVbExtSamp8VTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pVbExtSamp8VTLogitBoost::update_F(int t)
{
...
void pVbExtSamp8VTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
void pVbExtSamp8VTLogitBoost::update_gg()
{
  int N = F_.rows;
  double delta = 0;
...
//bool pVbExtSamp8VTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void pVbExtSamp8VTLogitBoost::calc_loss( MLData* _data )
{
...
void pVbExtSamp8VTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pVbExtSamp8VTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
