...
SampVTSplit::SampVTSplit()
{
  reset();
}

void SampVTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
SampVTNode::SampVTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int SampVTNode::calc_dir( float* _psample )
{
...
// Implementation of SampVTSolver
//const double SampVTSolver::EPS = 0.01;
const double SampVTSolver::MAXGAMMA = 5.0;
SampVTSolver::SampVTSolver( SampVTData*  _data)
{ 
  set_data(_data);
}

void SampVTSolver::set_data( SampVTData*  _data)
{ 
  data_ = _data;
...
void SampVTSolver::update_internal( VecIdx& vidx )
{
...
void SampVTSolver::update_internal_incre( int idx )
{
...
void SampVTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void SampVTSolver::calc_gamma( double *gamma)
{
  int K = mg_.size();
  for (int kk = 0; kk < K; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
     //if (sh <= 0) cv::error("SampVTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void SampVTSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
// Implementation of SampVTTree::Param
SampVTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
}
// Implementation of SampVTTree
void SampVTTree::split( SampVTData* _data )
{
  // clear
  clear();
  ...
  SampVTNode* root = candidate_nodes_.top(); 
  ...
  {
    SampVTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void SampVTTree::fit( SampVTData* _data )
{
  // fitting node data for each leaf
  std::list<SampVTNode>::iterator it;
...
SampVTNode* SampVTTree::get_node( float* _sample)
{
  SampVTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    SampVTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}

void SampVTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void SampVTTree::predict( float* _sample, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void SampVTTree::clear()
{
  nodes_.clear();
  candidate_nodes_.~priority_queue();
}

void SampVTTree::creat_root_node( SampVTData* _data )
{
  nodes_.push_back(SampVTNode(0,K_));
  SampVTNode* root = &(nodes_.back());
...
bool SampVTTree::find_best_candidate_split( SampVTNode* _node, SampVTData* _data )
{
...
bool SampVTTree::find_best_split_num_var( 
  SampVTNode* _node, SampVTData* _data, int _ivar)
{
  ...
  // initialize
  SampVTSolver sol_left(_data), sol_right = _node->sol_this_;
...
void SampVTTree::make_node_sorted_idx( SampVTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool SampVTTree::set_best_split_num_var( 
  SampVTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
...
bool SampVTTree::can_split_node( SampVTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool SampVTTree::split_node( SampVTNode* _node, SampVTData* _data )
{
  // create left and right node
  SampVTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  SampVTNode tmp2(nodes_.size(), K_);
  // 
...
void SampVTTree::calc_gain(SampVTNode* _node, SampVTData* _data)
{
  double gain;
...
void SampVTTree::fit_node( SampVTNode* _node, SampVTData* _data )
{
  int nn = _node->sample_idx_.size();
  CV_Assert(nn>0);
...
// Implementation of SampVTLogitBoost::Param
SampVTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of SampVTLogitBoost
const double SampVTLogitBoost::EPS_LOSS = 1e-14;
const double SampVTLogitBoost::MAX_F = 100;
void SampVTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void SampVTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void SampVTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void SampVTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void SampVTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int SampVTLogitBoost::get_class_count()
{
  return K_;
}

int SampVTLogitBoost::get_num_iter()
{
  return NumIter_;
}

//double SampVTLogitBoost::get_train_loss()
//{
//  if (NumIter_<1) return DBL_MAX;
//  return L_iter_.at<double>(NumIter_-1);
//}

void SampVTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void SampVTLogitBoost::update_F(int t)
{
...
void SampVTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
//bool SampVTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void SampVTLogitBoost::calc_loss( MLData* _data )
{
...
void SampVTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void SampVTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
