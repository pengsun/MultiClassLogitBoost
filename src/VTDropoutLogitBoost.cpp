#include "VTDropoutLogitBoost.hpp"
...
// Implementation of VTDropoutLogitSplit
VTDropoutLogitSplit::VTDropoutLogitSplit()
{
  reset();
}

void VTDropoutLogitSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
VTDropoutLogitNode::VTDropoutLogitNode(int _K)
{
  id_ = 0;
...
VTDropoutLogitNode::VTDropoutLogitNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int VTDropoutLogitNode::calc_dir( float* _psample )
{
...
// Implementation of VTDropoutLogitSolver
//const double VTDropoutLogitSolver::EPS = 0.01;
const double VTDropoutLogitSolver::MAXGAMMA = 5.0;
VTDropoutLogitSolver::VTDropoutLogitSolver( VTDropoutLogitData*  _data)
{ 
  data_ = _data;
...
void VTDropoutLogitSolver::update_internal( VecIdx& vidx )
{
...
void VTDropoutLogitSolver::update_internal_incre( int idx )
{
...
void VTDropoutLogitSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void VTDropoutLogitSolver::calc_gamma( double *gamma)
{
  int K = mg_.size();
  for (int kk = 0; kk < K; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
     //if (sh <= 0) cv::error("VTDropoutLogitSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void VTDropoutLogitSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
// Implementation of VTDropoutLogitTree::Param
VTDropoutLogitTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
}
// Implementation of VTDropoutLogitTree
void VTDropoutLogitTree::split( VTDropoutLogitData* _data )
{
  // clear
  clear();
  ...
  VTDropoutLogitNode* root = candidate_nodes_.top(); 
  ...
  {
    VTDropoutLogitNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void VTDropoutLogitTree::fit( VTDropoutLogitData* _data )
{
  // fitting node data for each leaf
  std::list<VTDropoutLogitNode>::iterator it;
  for (it = nodes_.begin(); it != nodes_.end(); ++it) {
    VTDropoutLogitNode* nd = &(*it);
...
VTDropoutLogitNode* VTDropoutLogitTree::get_node( float* _sample)
{
  VTDropoutLogitNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    VTDropoutLogitNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}
void VTDropoutLogitTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void VTDropoutLogitTree::predict( float* _sample, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void VTDropoutLogitTree::clear()
{
  nodes_.clear();
  candidate_nodes_.~priority_queue();
}

void VTDropoutLogitTree::creat_root_node( VTDropoutLogitData* _data )
{
  nodes_.push_back(VTDropoutLogitNode(0,K_));
  VTDropoutLogitNode* root = &(nodes_.back());
...
bool VTDropoutLogitTree::find_best_candidate_split( VTDropoutLogitNode* _node, VTDropoutLogitData* _data )
{
...
bool VTDropoutLogitTree::find_best_split_num_var( 
  VTDropoutLogitNode* _node, VTDropoutLogitData* _data, int _ivar)
{
  ...
  // initialize
  VTDropoutLogitSolver sol_left(_data), sol_right(_data);
...
void VTDropoutLogitTree::make_node_sorted_idx( VTDropoutLogitNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool VTDropoutLogitTree::set_best_split_num_var( 
  VTDropoutLogitNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
...
bool VTDropoutLogitTree::can_split_node( VTDropoutLogitNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool VTDropoutLogitTree::split_node( VTDropoutLogitNode* _node, VTDropoutLogitData* _data )
{
  // create left and right node
  VTDropoutLogitNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  VTDropoutLogitNode tmp2(nodes_.size(), K_);
  // 
...
void VTDropoutLogitTree::calc_gain(VTDropoutLogitNode* _node, VTDropoutLogitData* _data)
{
  VTDropoutLogitSolver sol(_data);
...
void VTDropoutLogitTree::fit_node( VTDropoutLogitNode* _node, VTDropoutLogitData* _data )
{
  int nn = _node->sample_idx_.size();
  CV_Assert(nn>0);
...
// Implementation of VTDropoutLogitBoost::Param
VTDropoutLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of VTDropoutLogitBoost
const double VTDropoutLogitBoost::EPS_LOSS = 1e-14;
const double VTDropoutLogitBoost::MAX_F = 100;
void VTDropoutLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void VTDropoutLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void VTDropoutLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void VTDropoutLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void VTDropoutLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int VTDropoutLogitBoost::get_class_count()
{
  return K_;
}

int VTDropoutLogitBoost::get_num_iter()
{
  return NumIter_;
}

//double VTDropoutLogitBoost::get_train_loss()
//{
//  if (NumIter_<1) return DBL_MAX;
//  return L_iter_.at<double>(NumIter_-1);
//}

void VTDropoutLogitBoost::train_init( MLData* _data )
{
  // class count
...
void VTDropoutLogitBoost::update_F(int t)
{
...
void VTDropoutLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
//bool VTDropoutLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void VTDropoutLogitBoost::calc_loss( MLData* _data )
{
...
void VTDropoutLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void VTDropoutLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
