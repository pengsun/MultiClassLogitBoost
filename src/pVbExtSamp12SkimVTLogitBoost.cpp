#include "pVbExtSamp12SkimVTLogitBoost.hpp"
#include <functional>
#include <numeric>
...
// Implementation of pVbExtSamp12SkimVTSplit
pVbExtSamp12SkimVTSplit::pVbExtSamp12SkimVTSplit()
{
  reset();
}

void pVbExtSamp12SkimVTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pVbExtSamp12SkimVTSolver
//const double pVbExtSamp12SkimVTSolver::EPS = 0.01;
const double pVbExtSamp12SkimVTSolver::MAXGAMMA = 5.0;
pVbExtSamp12SkimVTSolver::pVbExtSamp12SkimVTSolver( pVbExtSamp12SkimVTData* _data, VecIdx* _ci)
{ 
  set_data(_data, _ci);
}

void pVbExtSamp12SkimVTSolver::set_data( pVbExtSamp12SkimVTData* _data, VecIdx* _ci)
{ 
  data_ = _data;
  ci_ = _ci;
...
void pVbExtSamp12SkimVTSolver::update_internal( VecIdx& vidx )
{
...
void pVbExtSamp12SkimVTSolver::update_internal_incre( int idx )
{
...
void pVbExtSamp12SkimVTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pVbExtSamp12SkimVTSolver::calc_gamma( double *gamma)
{
  int KK = mg_.size();
  for (int kk = 0; kk < KK; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
    //if (sh <= 0) cv::error("pVbExtSamp12SkimVTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void pVbExtSamp12SkimVTSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
// Implementation of pVbExtSamp12SkimVTNode
pVbExtSamp12SkimVTNode::pVbExtSamp12SkimVTNode(int _K)
{
  id_ = 0;
...
pVbExtSamp12SkimVTNode::pVbExtSamp12SkimVTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pVbExtSamp12SkimVTNode::calc_dir( float* _psample )
{
...
// Implementation of pVbExt12SkimVT_best_split_finder (helper class)
pVbExt12SkimVT_best_split_finder::pVbExt12SkimVT_best_split_finder(pVbExtSamp12SkimVTTree *_tree, 
  pVbExtSamp12SkimVTNode *_node, pVbExtSamp12SkimVTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
pVbExt12SkimVT_best_split_finder::pVbExt12SkimVT_best_split_finder (const pVbExt12SkimVT_best_split_finder &f, cv::Split)
{
  this->tree_ = f.tree_;
  this->node_ = f.node_;
  this->data_ = f.data_;
...
void pVbExt12SkimVT_best_split_finder::operator() (const cv::BlockedRange &r)
{
  ...
  for (int ii = r.begin(); ii != r.end(); ++ii) {
    ...
    pVbExtSamp12SkimVTSplit the_split;
    the_split.reset();
    bool ret;
...
void pVbExt12SkimVT_best_split_finder::join (pVbExt12SkimVT_best_split_finder &rhs)
{
...
// Implementation of pVbExtSamp12SkimVTTree::Param
pVbExtSamp12SkimVTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
...
// Implementation of pVbExtSamp12SkimVTTree
void pVbExtSamp12SkimVTTree::split( pVbExtSamp12SkimVTData* _data )
{
  // clear
  clear();
  ...
  pVbExtSamp12SkimVTNode* root = candidate_nodes_.top(); 
  ...
  {
    pVbExtSamp12SkimVTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void pVbExtSamp12SkimVTTree::fit( pVbExtSamp12SkimVTData* _data )
{
  // fitting node data for each leaf
  std::list<pVbExtSamp12SkimVTNode>::iterator it;
  for (it = nodes_.begin(); it != nodes_.end(); ++it) {
    pVbExtSamp12SkimVTNode* nd = &(*it);
...
pVbExtSamp12SkimVTNode* pVbExtSamp12SkimVTTree::get_node( float* _sample)
{
  pVbExtSamp12SkimVTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pVbExtSamp12SkimVTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}
...
void pVbExtSamp12SkimVTTree::get_is_leaf( VecInt& is_leaf )
{
  is_leaf.clear();
  is_leaf.resize(nodes_.size());

  std::list<pVbExtSamp12SkimVTNode>::iterator it = nodes_.begin();
...
void pVbExtSamp12SkimVTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pVbExtSamp12SkimVTTree::predict( float* _sample, float* _score )
{
  ...
  pVbExtSamp12SkimVTNode* nd = get_node(_sample);
  for (int k = 0; k < K_; ++k) {
...
void pVbExtSamp12SkimVTTree::subsample_samples(pVbExtSamp12SkimVTData* _data)
{
...
void pVbExtSamp12SkimVTTree::subsample_classes_for_node( pVbExtSamp12SkimVTNode* _node, pVbExtSamp12SkimVTData* _data )
{
...
void pVbExtSamp12SkimVTTree::clear()
{
  nodes_.clear();
...
void pVbExtSamp12SkimVTTree::creat_root_node( pVbExtSamp12SkimVTData* _data )
{
  nodes_.push_back(pVbExtSamp12SkimVTNode(0,K_));
  pVbExtSamp12SkimVTNode* root = &(nodes_.back());
...
bool pVbExtSamp12SkimVTTree::find_best_candidate_split( pVbExtSamp12SkimVTNode* _node, pVbExtSamp12SkimVTData* _data )
{
  ...
  pVbExt12SkimVT_best_split_finder bsf(this,_node,_data);
...
bool pVbExtSamp12SkimVTTree::find_best_split_num_var( 
  pVbExtSamp12SkimVTNode* _node, pVbExtSamp12SkimVTData* _data, int _ivar, pVbExtSamp12SkimVTSplit &cb_split)
{
  ...
  // initialize
  pVbExtSamp12SkimVTSolver sol_left(_data, _node->sol_this_.ci_), 
...
void pVbExtSamp12SkimVTTree::make_node_sorted_idx( pVbExtSamp12SkimVTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pVbExtSamp12SkimVTTree::set_best_split_num_var( 
  pVbExtSamp12SkimVTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
...
bool pVbExtSamp12SkimVTTree::can_split_node( pVbExtSamp12SkimVTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pVbExtSamp12SkimVTTree::split_node( pVbExtSamp12SkimVTNode* _node, pVbExtSamp12SkimVTData* _data )
{
  // create left and right node
  pVbExtSamp12SkimVTNode tmp1(nodes_.size(), K_);
  nodes_.push_back(tmp1);
  _node->left_ = &(nodes_.back());
  _node->left_->parent_ = _node;

  pVbExtSamp12SkimVTNode tmp2(nodes_.size(), K_);
...
void pVbExtSamp12SkimVTTree::calc_gain(pVbExtSamp12SkimVTNode* _node, pVbExtSamp12SkimVTData* _data)
{
  double gain;
...
void pVbExtSamp12SkimVTTree::fit_node( pVbExtSamp12SkimVTNode* _node, pVbExtSamp12SkimVTData* _data )
{
  int nn = _node->sample_idx_.size();
  if (nn<=0) return;
  ...
  for (int k = 0; k < this->K_; ++k)
    ci[k] = k;
  pVbExtSamp12SkimVTSolver sol(_data, &ci);
...
// Implementation of pVbExtSamp12SkimVTLogitBoost::Param
pVbExtSamp12SkimVTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pVbExtSamp12SkimVTLogitBoost
const double pVbExtSamp12SkimVTLogitBoost::EPS_LOSS = 1e-6;
const double pVbExtSamp12SkimVTLogitBoost::MAX_F = 100;
void pVbExtSamp12SkimVTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pVbExtSamp12SkimVTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pVbExtSamp12SkimVTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pVbExtSamp12SkimVTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pVbExtSamp12SkimVTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pVbExtSamp12SkimVTLogitBoost::get_class_count()
{
  return K_;
}

int pVbExtSamp12SkimVTLogitBoost::get_num_iter()
{
  return NumIter_;
}

void pVbExtSamp12SkimVTLogitBoost::get_nr( VecIdx& nr_wts, VecIdx& nr_wtc )
{
...
void pVbExtSamp12SkimVTLogitBoost::get_cc( int itree, VecInt& node_cc )
{
...
void pVbExtSamp12SkimVTLogitBoost::get_sc( int itree, VecInt& node_sc )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
//double pVbExtSamp12SkimVTLogitBoost::get_train_loss()
//{
...
void pVbExtSamp12SkimVTLogitBoost::get_is_leaf( int itree, VecInt& is_leaf )
{
  if (NumIter_==0) return;
  if (itree<0) itree = 0;
  if ( (itree+1) >= NumIter_ ) itree = (NumIter_-1);
...
void pVbExtSamp12SkimVTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pVbExtSamp12SkimVTLogitBoost::update_F(int t)
{
...
void pVbExtSamp12SkimVTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
void pVbExtSamp12SkimVTLogitBoost::update_gg()
{
  int N = F_.rows;
  double delta = 0;
...
//bool pVbExtSamp12SkimVTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void pVbExtSamp12SkimVTLogitBoost::calc_loss( MLData* _data )
{
...
void pVbExtSamp12SkimVTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pVbExtSamp12SkimVTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
