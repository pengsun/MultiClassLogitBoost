...
// Solver
struct pVbExtSamp6VTSolver {
  static const double MAXGAMMA;
  //static const double EPS;

  pVbExtSamp6VTSolver () {};
  pVbExtSamp6VTSolver (pVbExtSamp6VTData* _data, VecIdx* _ci);
  void set_data (pVbExtSamp6VTData* _data, VecIdx* _ci);
...
public:
  ...
  pVbExtSamp6VTData* data_;
  VecIdx *ci_;
};

// Split descriptor
struct pVbExtSamp6VTSplit {
public:
  pVbExtSamp6VTSplit ();
  void reset ();
...
struct pVbExtSamp6VTNode {
public:
  pVbExtSamp6VTNode (int _K);
  pVbExtSamp6VTNode (int _id, int _K);
...
public:
  ...
  pVbExtSamp6VTNode *parent_, *left_, *right_; //
  pVbExtSamp6VTSplit split_;

  VecIdx sample_idx_; // for all the examples this node holds

  pVbExtSamp6VTSolver sol_this_; // for all the examples this node holds
};

// Node Comparator: the less the expected gain, the less the node
struct pVbExtSamp6VTNodeLess {
  bool operator () (const pVbExtSamp6VTNode* n1, const pVbExtSamp6VTNode* n2) {
...
typedef std::priority_queue<pVbExtSamp6VTNode*, 
                            std::vector<pVbExtSamp6VTNode*>, 
                            pVbExtSamp6VTNodeLess>
        QuepVbExtSamp6VTNode;

// Best Split Finder (helper class for parallel_reduce) 
class pVbExtSamp6VTTree;
struct pVbExt5VT_best_split_finder {
  pVbExt5VT_best_split_finder (pVbExtSamp6VTTree *_tree, 
    pVbExtSamp6VTNode* _node, pVbExtSamp6VTData* _data); // customized data
  ...
  pVbExtSamp6VTTree *tree_;
  pVbExtSamp6VTNode *node_;
  pVbExtSamp6VTData *data_;
  pVbExtSamp6VTSplit cb_split_;
};
...
public:
  void split( pVbExtSamp6VTData* _data );
  void fit ( pVbExtSamp6VTData* _data );

  pVbExtSamp6VTNode* get_node (float* _sample);
  void predict (MLData* _data);
  void predict (float* _sample, float* _score);

public:
  void subsample (pVbExtSamp6VTData* _data);

  void clear ();
  void creat_root_node (pVbExtSamp6VTData* _data);

  virtual bool find_best_candidate_split (pVbExtSamp6VTNode* _node, pVbExtSamp6VTData* _data);
  virtual bool find_best_split_num_var (pVbExtSamp6VTNode* _node, pVbExtSamp6VTData* _data, int _ivar,
                                        pVbExtSamp6VTSplit &spl);

  void make_node_sorted_idx(pVbExtSamp6VTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node);
  bool set_best_split_num_var ( pVbExtSamp6VTNode* _node, MLData* _data, int _ivar, 
    VecIdx& node_sample_si,
    int best_i, double best_gain, double best_gain_left, double best_gain_right,
    pVbExtSamp6VTSplit &cb_split);

  bool can_split_node (pVbExtSamp6VTNode* _node);
  bool split_node (pVbExtSamp6VTNode* _node, pVbExtSamp6VTData* _data);
  void calc_gain (pVbExtSamp6VTNode* _node, pVbExtSamp6VTData* _data);
  virtual void fit_node (pVbExtSamp6VTNode* _node, pVbExtSamp6VTData* _data);

protected:
  std::list<pVbExtSamp6VTNode> nodes_; // all nodes
  QuepVbExtSamp6VTNode candidate_nodes_; // priority queue of candidate leaves for splitting
  // cb: current best
...
// vector of pVbExtSamp6VTTree
typedef std::vector<pVbExtSamp6VTTree> VecpVbExtSamp6VTTree;

// Boost
class pVbExtSamp6VTLogitBoost {
public:
...
protected:
  ...
  pVbExtSamp6VTData logitdata_;
  VecpVbExtSamp6VTTree trees_;
