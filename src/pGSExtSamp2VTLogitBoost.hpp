...
// Solver
struct pGSExtSamp2VTSolver {
  static const double MAXGAMMA;
  //static const double EPS;

  pGSExtSamp2VTSolver () {};
  pGSExtSamp2VTSolver (pGSExtSamp2VTData* _data, VecIdx* _ci);
  void set_data (pGSExtSamp2VTData* _data, VecIdx* _ci);
...
public:
  ...
  pGSExtSamp2VTData* data_;
  VecIdx *ci_;
};

// Split descriptor
struct pGSExtSamp2VTSplit {
public:
  pGSExtSamp2VTSplit ();
  void reset ();
...
struct pGSExtSamp2VTNode {
public:
  pGSExtSamp2VTNode (int _K);
  pGSExtSamp2VTNode (int _id, int _K);
...
public:
  ...
  pGSExtSamp2VTNode *parent_, *left_, *right_; //
  pGSExtSamp2VTSplit split_;

  VecIdx sample_idx_; // for all the examples this node holds

  pGSExtSamp2VTSolver sol_this_; // for all the examples this node holds
};

// Node Comparator: the less the expected gain, the less the node
struct pGSExtSamp2VTNodeLess {
  bool operator () (const pGSExtSamp2VTNode* n1, const pGSExtSamp2VTNode* n2) {
...
typedef std::priority_queue<pGSExtSamp2VTNode*, 
                            std::vector<pGSExtSamp2VTNode*>, 
                            pGSExtSamp2VTNodeLess>
        QuepGSExtSamp2VTNode;

// Best Split Finder (helper class for parallel_reduce) 
class pGSExtSamp2VTTree;
struct pGSExt2VT_best_split_finder {
  pGSExt2VT_best_split_finder (pGSExtSamp2VTTree *_tree, 
    pGSExtSamp2VTNode* _node, pGSExtSamp2VTData* _data); // customized data
  pGSExt2VT_best_split_finder (const pGSExt2VT_best_split_finder &f, cv::Split); // required

  void operator () (const cv::BlockedRange &r); // required
  void join (pGSExt2VT_best_split_finder &rhs); // required

  pGSExtSamp2VTTree *tree_;
  pGSExtSamp2VTNode *node_;
  pGSExtSamp2VTData *data_;
  pGSExtSamp2VTSplit cb_split_;
};
...
public:
  void split( pGSExtSamp2VTData* _data );
  void fit ( pGSExtSamp2VTData* _data );

  pGSExtSamp2VTNode* get_node (float* _sample);
  void predict (MLData* _data);
  void predict (float* _sample, float* _score);

public:
  void subsample (pGSExtSamp2VTData* _data);

  void clear ();
  void creat_root_node (pGSExtSamp2VTData* _data);

  virtual bool find_best_candidate_split (pGSExtSamp2VTNode* _node, pGSExtSamp2VTData* _data);
  virtual bool find_best_split_num_var (pGSExtSamp2VTNode* _node, pGSExtSamp2VTData* _data, int _ivar,
                                        pGSExtSamp2VTSplit &spl);

  void make_node_sorted_idx(pGSExtSamp2VTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node);
  bool set_best_split_num_var ( pGSExtSamp2VTNode* _node, MLData* _data, int _ivar, 
    VecIdx& node_sample_si,
    int best_i, double best_gain, double best_gain_left, double best_gain_right,
    pGSExtSamp2VTSplit &cb_split);

  bool can_split_node (pGSExtSamp2VTNode* _node);
  bool split_node (pGSExtSamp2VTNode* _node, pGSExtSamp2VTData* _data);
  void calc_gain (pGSExtSamp2VTNode* _node, pGSExtSamp2VTData* _data);
  virtual void fit_node (pGSExtSamp2VTNode* _node, pGSExtSamp2VTData* _data);

protected:
  std::list<pGSExtSamp2VTNode> nodes_; // all nodes
  QuepGSExtSamp2VTNode candidate_nodes_; // priority queue of candidate leaves for splitting
  // cb: current best
...
// vector of pGSExtSamp2VTTree
typedef std::vector<pGSExtSamp2VTTree> VecpGSExtSamp2VTTree;

// Boost
class pGSExtSamp2VTLogitBoost {
public:
...
protected:
  ...
  pGSExtSamp2VTData logitdata_;
  VecpGSExtSamp2VTTree trees_;
