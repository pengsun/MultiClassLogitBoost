...
// Implementation of pConsVTSplit
pConsVTSplit::pConsVTSplit()
{
  reset();
}

void pConsVTSplit::reset()
{
  var_idx_ = -1;
  threshold_ = FLT_MAX;
  subset_.reset();
...
// Implementation of pConsVTSolver
//const double pConsVTSolver::EPS = 0.01;
const double pConsVTSolver::MAXGAMMA = 5.0;
pConsVTSolver::pConsVTSolver( pConsVTData*  _data)
{ 
  set_data(_data);
}

void pConsVTSolver::set_data( pConsVTData*  _data)
{ 
  data_ = _data;
...
void pConsVTSolver::update_internal( VecIdx& vidx )
{
...
void pConsVTSolver::update_internal_incre( int idx )
{
...
void pConsVTSolver::update_internal_decre( int idx )
{
  int yi = int( data_->data_cls_->Y.at<float>(idx) );
  double* ptr_pi = data_->p_->ptr<double>(idx);
...
void pConsVTSolver::calc_gamma( double *gamma)
{
  int K = mg_.size();
  for (int kk = 0; kk < K; ++kk) {
    double smg = mg_[kk];
    double sh  = h_[kk];
    //if (sh <= 0) cv::error("pConsVTSolver::calc_gamma: Invalid Hessian.");
    if (sh == 0) sh = 1;
...
void pConsVTSolver::calc_gain( double& gain )
{
  gain = 0;
  int KK = mg_.size();
  for (int k = 0; k < KK; ++k) {
...
pConsVTNode::pConsVTNode(int _K)
{
  id_ = 0;
...
pConsVTNode::pConsVTNode( int _id, int _K )
{
  id_ = _id;
  parent_ = left_ = right_ = 0;
...
int pConsVTNode::calc_dir( float* _psample )
{
...
// Implementation of pConsVT_best_split_finder (helper class)
pConsVT_best_split_finder::pConsVT_best_split_finder(pConsVTTree *_tree, pConsVTNode *_node, pConsVTData *_data)
{
  this->tree_ = _tree;
  this->node_ = _node;
  this->data_ = _data;
...
pConsVT_best_split_finder::pConsVT_best_split_finder (const pConsVT_best_split_finder &f, cv::Split)
{
  this->tree_ = f.tree_;
  this->node_ = f.node_;
  this->data_ = f.data_;
...
void pConsVT_best_split_finder::operator() (const cv::BlockedRange &r)
{

  // for each variable, find the best split
  for (int vi = r.begin(); vi != r.end(); ++vi) {
    pConsVTSplit the_split;
    the_split.reset();
    bool ret;
...
void pConsVT_best_split_finder::join (pConsVT_best_split_finder &rhs)
{
...
// Implementation of pConsVTTree::Param
pConsVTTree::Param::Param()
{
  max_leaves_ = 2;
  node_size_ = 5;
}
...
void pConsVTTree::split( pConsVTData* _data )
{
  ...
  pConsVTNode* root = candidate_nodes_.top(); 
  ...
  {
    pConsVTNode* cur_node = candidate_nodes_.top(); // the most prior node
...
void pConsVTTree::fit( pConsVTData* _data )
{
  ...
  std::list<pConsVTNode>::iterator it;
...
pConsVTNode* pConsVTTree::get_node( float* _sample)
{
  pConsVTNode* cur_node = &(nodes_.front());
  while (true) {
    if (cur_node->left_==0) break; // leaf reached 

    int dir = cur_node->calc_dir(_sample);
    pConsVTNode* next = (dir==-1) ? (cur_node->left_) : (cur_node->right_);
    cur_node = next;
  }
  return cur_node;
}
void pConsVTTree::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
...
void pConsVTTree::predict( float* _sample, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pConsVTTree::clear()
{
  nodes_.clear();
  candidate_nodes_.~priority_queue();
}

void pConsVTTree::creat_root_node( pConsVTData* _data )
{
  nodes_.push_back(pConsVTNode(0,K_));
  pConsVTNode* root = &(nodes_.back());
...
bool pConsVTTree::find_best_candidate_split( pConsVTNode* _node, pConsVTData* _data )
{
  ...
  pConsVT_best_split_finder bsf(this,_node,_data);
...
void pConsVTTree::make_node_sorted_idx( pConsVTNode* _node, MLData* _data, int _ivar, VecIdx& sorted_idx_node )
{
  VecIdx16 sam_idx16;
  VecIdx32 sam_idx32;
  if (_data->is_idx16) 
...
bool pConsVTTree::set_best_split_num_var( 
  pConsVTNode* _node, MLData* _data, int _ivar, 
  VecIdx& node_sample_si, 
...
bool pConsVTTree::can_split_node( pConsVTNode* _node )
{
  bool flag = true;
  int nn = _node->sample_idx_.size();
...
bool pConsVTTree::split_node( pConsVTNode* _node, pConsVTData* _data )
{
  // create left and right node
  pConsVTNode tmp1(nodes_.size(), K_);
...
void pConsVTTree::fit_node( pConsVTNode* _node, pConsVTData* _data )
{
  int nn = _node->sample_idx_.size();
  CV_Assert(nn>0);
...
// Implementation of pConsVTLogitBoost::Param
pConsVTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pConsVTLogitBoost
const double pConsVTLogitBoost::EPS_LOSS = 1e-14;
const double pConsVTLogitBoost::MAX_F = 100;
void pConsVTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pConsVTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pConsVTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pConsVTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pConsVTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pConsVTLogitBoost::get_class_count()
{
  return K_;
}

int pConsVTLogitBoost::get_num_iter()
{
  return NumIter_;
}

//double pConsVTLogitBoost::get_train_loss()
//{
//  if (NumIter_<1) return DBL_MAX;
//  return L_iter_.at<double>(NumIter_-1);
//}

void pConsVTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pConsVTLogitBoost::update_F(int t)
{
...
void pConsVTLogitBoost::update_p()
{
  int N = F_.rows;
  int K = K_;
...
//bool pConsVTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void pConsVTLogitBoost::calc_loss( MLData* _data )
{
...
void pConsVTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pConsVTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
