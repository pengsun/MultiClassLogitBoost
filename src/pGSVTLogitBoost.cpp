...
// Implementation of pGSVTLogitBoost::Param
pGSVTLogitBoost::Param::Param()
{
  T = 2;
  v = 0.1;
  J = 4;
...
// Implementation of pGSVTLogitBoost
const double pGSVTLogitBoost::EPS_LOSS = 1e-14;
const double pGSVTLogitBoost::MAX_F = 100;
void pGSVTLogitBoost::train( MLData* _data )
{
  train_init(_data);
...
void pGSVTLogitBoost::predict( MLData* _data )
{
  int N = _data->X.rows;
  int K = K_;
  if (_data->Y.rows!=N || _data->Y.cols!=K)
...
void pGSVTLogitBoost::predict( float* _sapmle, float* _score )
{
  // initialize
  for (int k = 0; k < K_; ++k) {
    *(_score+k) = 0;
...
void pGSVTLogitBoost::predict( MLData* _data, int _Tpre )
{
  // trees to be used
...
void pGSVTLogitBoost::predict( float* _sapmle, float* _score, int _Tpre )
{
...
int pGSVTLogitBoost::get_class_count()
{
  return K_;
}

int pGSVTLogitBoost::get_num_iter()
{
  return NumIter_;
}

//double pGSVTLogitBoost::get_train_loss()
//{
//  if (NumIter_<1) return DBL_MAX;
//  return L_iter_.at<double>(NumIter_-1);
//}

void pGSVTLogitBoost::train_init( MLData* _data )
{
  // class count
...
void pGSVTLogitBoost::update_F(int t)
{
...
//bool pGSVTLogitBoost::should_stop( int t )
//{
//  int N = F_.rows;
...
void pGSVTLogitBoost::calc_loss( MLData* _data )
{
...
void pGSVTLogitBoost::calc_loss_iter( int t )
{
  double sum = 0;
  int N = L_.rows;
  for (int i = 0; i < N; ++i) 
...
void pGSVTLogitBoost::calc_grad( int t )
{
  int N = F_.rows;
  double delta = 0;
