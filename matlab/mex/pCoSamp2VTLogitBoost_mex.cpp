...
#include "./../../src/pCoSamp2VTLogitBoost.hpp"
#include "utilCPP.h"
#include <cstring>

// typedef for booster
typedef pCoSamp2VTLogitBoost booster_t;

namespace {
  
}

// h = train(dummy, X,Y, var_cat_mask, T,J,v, node_size, rb,rf, wrb);
void train(int nlhs,mxArray *plhs[], int nrhs,const mxArray *prhs[]) 
{
  ...
  pbooster->param_.rb_ = rb;
  pbooster->param_.rf_ = rf;
  pbooster->param_.wrb_ = wrb;
...
void get_cc(int nlhs,mxArray *plhs[], int nrhs,const mxArray *prhs[]) 
{
  ...
  /*Output*/
  plhs[0] = VecDbl_to_mxArray(node_cc);
}
