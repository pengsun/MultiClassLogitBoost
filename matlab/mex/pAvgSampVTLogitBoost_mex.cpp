...
#include "./../../src/pAvgSampVTLogitBoost.hpp"
#include "utilCPP.h"
#include <cstring>

// typedef for booster
typedef pAvgSampVTLogitBoost booster_t;

namespace {
  
}

// h = train(dummy, X,Y, var_cat_mask, T,J,v, node_size, rs,rf,rc, wrs,wrc, Tdot);
void train(int nlhs,mxArray *plhs[], int nrhs,const mxArray *prhs[]) 
{
  ...
  // weight ratio sample
  double weight_ratio_si = (double)mxGetScalar(prhs[11]);
  // weight ratio class
  double weight_ratio_ci = (double)mxGetScalar(prhs[12]);
  // Tdot
  int Tdot = (int)mxGetScalar(prhs[13]);
  ...
  pbooster->param_.weight_ratio_ci_ = weight_ratio_ci;

  pbooster->param_.Tdot = Tdot;
...
// [node_sc, node_all_sc] = get_sc(dummy, h, itree);
void get_sc(int nlhs,mxArray *plhs[], int nrhs,const mxArray *prhs[]) 
{
  ...
  VecInt node_all_sc;
  pbooster->get_all_sc(itree, node_all_sc);

  /*Output*/
  plhs[0] = VecInt_to_mxArray(node_sc);
  plhs[1] = VecInt_to_mxArray(node_all_sc);
}
...
// [leaf_gain,leaf_allsample_gain] = get_gain(dummy, h, itree);
void get_gain(int nlhs,mxArray *plhs[], int nrhs,const mxArray *prhs[]) 
{
  /* Input */
  // get pointer
  double *ptmp = (double*)mxGetData(prhs[1]);
  ...
  //
  VecDbl leaf_gain;
  pbooster->get_gain(itree,leaf_gain);
  VecDbl leaf_allsample_gain;
  pbooster->get_allsample_gain(itree,leaf_allsample_gain);

  /*Output*/
  plhs[0] = VecDbl_to_mxArray(leaf_gain);
  plhs[1] = VecDbl_to_mxArray(leaf_allsample_gain);
}
