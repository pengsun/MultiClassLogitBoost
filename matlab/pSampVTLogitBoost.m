classdef pSampVTLogitBoost
  ...
  methods
    function obj = train(obj,X,Y, varargin)
      ...
      obj.ptr = pSampVTLogitBoost_mex('train',...
        X,Y,var_cat_mask,...
        T, J, v,...
        node_size,...
        rs,rf,rc);
    ...
    function [NumIter, TrLoss,F,P,tree] = get (obj)
      [NumIter, TrLoss, F,P] = pSampVTLogitBoost_mex('get',obj.ptr);
      tree = 0;
    end
    
    function Y = predict(obj, X, T)
      if (nargin==2) 
        T = pSampVTLogitBoost_mex('get',obj.ptr);
      end
      Y = pSampVTLogitBoost_mex('predict',obj.ptr, X, T);
    end
    
    function delete(obj)
      pSampVTLogitBoost_mex('delete',obj.ptr);
    end
  end % method
