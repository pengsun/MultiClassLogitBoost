classdef pAOSOGradBoost
  ...
  methods
    function obj = train(obj,X,Y, varargin)
      ...
      obj.ptr = pAOSOGradBoost_mex('train',...
        X,Y,var_cat_mask,...
        T, J, v,...
        node_size,...
        rs,rf,wrs);
    ...
    function [NumIter, TrLoss,F,P,tree,...
      ...
      [NumIter, TrLoss, F,P, GradCls,LossCls] =...
        pAOSOGradBoost_mex('get',obj.ptr);
      tree = 0;
    end    
    
    function tree_node_cc = get_cc(obj)
      NumIter = pAOSOGradBoost_mex('get',obj.ptr);
      for i = 1 : NumIter
        tree_node_cc{i} = ...
          pAOSOGradBoost_mex('get_cc',obj.ptr, i); %#ok<AGROW>
      end
    end
    ...
    function tree_node_sc = get_sc(obj)
      ...
      for i = 1 : NumIter
        tree_node_sc{i} = ...
          pAOSOGradBoost_mex('get_sc',obj.ptr, i); %#ok<AGROW>
      end
    end    
    ...
    function tree_is_leaf = get_is_leaf(obj)
      ...
      for i = 1 : NumIter
        tree_is_leaf{i} = ...
          pAOSOGradBoost_mex('get_is_leaf',obj.ptr, i); %#ok<AGROW>
      end % for i
    end % get_is_leaf    
    
    function Y = predict(obj, X, T)
      if (nargin==2) 
        T = pAOSOGradBoost_mex('get',obj.ptr);
      end
      Y = pAOSOGradBoost_mex('predict',obj.ptr, X, T);
    end
    
    function delete(obj)
      pAOSOGradBoost_mex('delete',obj.ptr);
    end
  end % method
