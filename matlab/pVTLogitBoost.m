classdef pVTLogitBoost
  ...
  methods
    function obj = train(obj,X,Y, varargin)
      ...
      obj.ptr = pVTLogitBoost_mex('train',...
        X,Y,var_cat_mask,...
        T, J, v,...
        node_size);
    end
    ...
    function [NumIter, TrLoss,F,P,tree] = get (obj)
      [NumIter, TrLoss, F,P] = pVTLogitBoost_mex('get',obj.ptr);
      tree = 0;
    end
    
    function Y = predict(obj, X, T)
      if (nargin==2) 
        T = pVTLogitBoost_mex('get',obj.ptr);
      end
      Y = pVTLogitBoost_mex('predict',obj.ptr, X, T);
    end
    
    function delete(obj)
      pVTLogitBoost_mex('delete',obj.ptr);
    end
  end % method
