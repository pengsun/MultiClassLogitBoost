classdef batch_avgsampboost_basic
  ...
  methods
    ...
    function [hboost, t_tr, num_it, abs_grad,...
        F, P, Trees,...
        tree_node_cc, tree_node_sc, tree_node_all_sc, tree_is_leaf,...
      ...
      tmp = obj.param_to_str(...
        ...
        obj.Tdot,...
        obj.rs,obj.rf,obj.rc,obj.wrs,obj.wrc);
      fprintf(tmp); fprintf('\n');
      ...
      hboost = train(hboost, Xtr,Ytr,...
        ...
        'node_size',obj.ns,...
        'rs',obj.rs, 'rf',obj.rf,'rc',obj.rc,'wrs',obj.wrs,'wrc',obj.wrc,...
        'Tdot',obj.Tdot);
      t_tr = toc;
      ...
      [tree_node_sc,tree_node_all_sc] = get_sc(hboost);
      tree_is_leaf = get_is_leaf(hboost);
      [tree_leaf_gain,tree_leaf_allsample_gain] = get_gain(hboost);
    end
          ...
          for ix4 = 1 : numel(obj.cns)
            ...
            for ix5 = 1 : numel(obj.crs)
              ...
              for ix6 = 1 : numel(obj.crf)
                obj.rf = obj.crf{ix6};
                for ix7 = 1 : numel(obj.crc)
                  obj.rc = obj.crc{ix7};
                  for ix8 = 1 :numel(obj.cwrs)
                  obj.wrs = obj.cwrs{ix8};
                    for ix9 = 1 : numel(obj.cwrc)
                    obj.wrc = obj.cwrc{ix9};  
                      for ix10 = 1 : numel(obj.cTdot)
                      obj.Tdot = obj.cTdot{ix10};
                    
                      % train
                      [hboost, time_tr, num_it, abs_grad,...
                        F, P,Trees,...
                        tree_node_cc,...
                        tree_node_sc,tree_node_all_sc,...
                        tree_is_leaf,...
                        tree_leaf_gain,tree_leaf_allsample_gain,...
                        GradCls, LossCls] = ...
  ...
  methods(Static)
    function ret = param_to_str(T,v,J,ns,Tdot,rs,rf,rc,wrs,wrc)
      tmpl = [...
        'T%d','_',...
        'v%1.1d','_',...
        'J%d','_',...
        ...
        'rc%1.2d'];
      ret = sprintf(tmpl,...
        T,v,J,ns, Tdot, wrs,rs, rf, wrc,rc);
    end % param_to_str
