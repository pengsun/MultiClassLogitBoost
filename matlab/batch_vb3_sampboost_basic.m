classdef batch_vb3_sampboost_basic
  ...
  methods
    ...
    function [hboost, t_tr, num_it, abs_grad,...
        F, P, Trees,...
        nr_wts, nr_wtc,...
        tree_node_cc, tree_node_sc] = run_train (obj, fn_data)
      ...
      tmp = obj.param_to_str(...
        obj.T, obj.v, obj.J, obj.ns, obj.rs,obj.rf,obj.rc);
      fprintf(tmp); fprintf('\n');
      ...
      hboost = train(hboost, Xtr,Ytr,...
        ...
        'node_size',obj.ns,...
        'rs',obj.rs, 'rf',obj.rf,'rc',obj.rc);
      t_tr = toc;
      fprintf('train end at %s\n', datestr(clock));      
      drawnow;
      
      % get train result
      [num_it,abs_grad, F, P, Trees, nr_wts, nr_wtc] = get(hboost);
  ...
  methods(Static)
    function ret = param_to_str(T,v,J,ns,rs,rf,rc)
      tmpl = [...
        'T%d','_',...
        'v%1.1d','_',...
        'J%d','_',...
