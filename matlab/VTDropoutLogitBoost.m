classdef VTDropoutLogitBoost
  ...
  methods
    function obj = train(obj,X,Y, varargin)
      ...
      obj.ptr = VTDropoutLogitBoost_mex('train',...
        X,Y,var_cat_mask,...
        T, J, v,...
        node_size);
    end
    ...
    function [NumIter, TrLoss,F,P,tree] = get (obj)
      [NumIter, TrLoss, F,P] = VTDropoutLogitBoost_mex('get',obj.ptr);
      tree = 0;
    end
    
    function Y = predict(obj, X, T)
      if (nargin==2) 
        T = VTDropoutLogitBoost_mex('get',obj.ptr);
      end
      Y = VTDropoutLogitBoost_mex('predict',obj.ptr, X, T);
    end
    
    function delete(obj)
      VTDropoutLogitBoost_mex('delete',obj.ptr);
    end
  end % method
