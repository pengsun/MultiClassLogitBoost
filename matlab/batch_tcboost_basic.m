classdef batch_tcboost_basic
  % Basic class for Totally-Corrective-Boosting's batch run
  %   Detailed explanation goes here
  
  properties
    num_Tpre = 4;
    T = 1;
    lambda = 1; % magnitude for L1 regularizer
    J = 2;
    ns = 1;
  ...
  methods
    ...
    function [hboost, t_tr, num_it, abs_grad, F, P, Trees] = run_train (obj, fn_data)
      ...
      hboost = train(hboost, Xtr,Ytr,...
        ...
        'T',obj.T, 'lambda',obj.lambda, 'J',obj.J,...
        'node_size',obj.ns);
      t_tr = toc;
    ...
    function run_all_param (obj, fn_data, dir_rst)
      ...
      for ix2 = 1 : numel(obj.clambda)
        obj.lambda = obj.clambda{ix2};
  ...
  methods(Static)
    function ret = param_to_str(T,lambda,J,ns)
      tmpl = [...
        'T%d','_',...
        'lambda%1.1d','_',...
        'J%d','_',...
    ...
    function ret = predinc_param_to_str(Tpre,lambda,J,ns)
     tmpl = [...
        'Tpre=%d',', ',...
        'lambda=%1.1d',', ',...
        'J=%d',', ',...
