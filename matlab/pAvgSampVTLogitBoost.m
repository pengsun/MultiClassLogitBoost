  ...
  methods
    function obj = train(obj,X,Y, varargin)
      [var_cat_mask,T,J,v, node_size,...
          rs,rf,rc, wrs,wrc,...
          Tdot] = parse_input(varargin{:});
      if (isempty(var_cat_mask))
        nvar = size(X,1);
        var_cat_mask = uint8( zeros(nvar,1) );
      end
      obj.ptr = pAvgSampVTLogitBoost_mex('train',...
        X,Y,var_cat_mask,...
        T, J, v,...
        node_size,...
        rs,rf,rc, wrs,wrc,...
    ...
    function [NumIter, TrLoss,F,P,tree,...
      ...
      [NumIter, TrLoss, F,P, GradCls,LossCls] =...
        pAvgSampVTLogitBoost_mex('get',obj.ptr);
      tree = 0;
    end
    
    function tree_node_cc = get_cc(obj)
      NumIter = pAvgSampVTLogitBoost_mex('get',obj.ptr);
      for i = 1 : NumIter
        tree_node_cc{i} = ...
          pAvgSampVTLogitBoost_mex('get_cc',obj.ptr, i); %#ok<AGROW>
      end
    end
    ...
    function [tree_node_sc, tree_node_all_sc] = get_sc(obj)
      ...
      for i = 1 : NumIter
        [tree_node_sc{i},tree_node_all_sc{i}] = ...
          pAvgSampVTLogitBoost_mex('get_sc',obj.ptr, i); %#ok<AGROW>
      end
    end    
    ...
    function [tree_leaf_gain, tree_leaf_allsample_gain] = get_gain(obj)
      ...
      for i = 1 : NumIter
        [tree_leaf_gain{i},tree_leaf_allsample_gain{i}] = ...
          pAvgSampVTLogitBoost_mex('get_gain',obj.ptr, i); %#ok<AGROW>
      end
    end
    ...
    function tree_is_leaf = get_is_leaf(obj)
      ...
      for i = 1 : NumIter
        tree_is_leaf{i} = ...
          pAvgSampVTLogitBoost_mex('get_is_leaf',obj.ptr, i); %#ok<AGROW>
      end % for i
    end % get_is_leaf
    
    function Y = predict(obj, X, T)
      if (nargin==2) 
        T = pAvgSampVTLogitBoost_mex('get',obj.ptr);
      end
      Y = pAvgSampVTLogitBoost_mex('predict',obj.ptr, X, T);
    end
    
    function delete(obj)
      pAvgSampVTLogitBoost_mex('delete',obj.ptr);
    end
  end % method
